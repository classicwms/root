...
public interface BillingRepository extends JpaRepository<Billing,String>, JpaSpecificationExecutor<Billing> {
    ...
    List<Billing> findByBillGenerationIndicatorAndBillModeIdAndDeletionIndicator(String automatic, Long billModeId, Long deletionIndicator);

    @Query(value = "select day_bill_gen from tblbillingfrequencyid where " +
            "c_id = :companyId and plant_id = :plantId and wh_id = :warehouseId and lang_id = :languageId and " +
            "bill_freq_id = :billId and is_deleted =0", nativeQuery = true)
    public String getDayBillDate(@Param("companyId") String companyId,
                               @Param("plantId") String plantId,
                               @Param("languageId") String languageId,
                               @Param("warehouseId") String warehouseId,
                               @Param("billId") Long billId);

    @Query(value = "select date_bill_gen from tblbillingfrequencyid where " +
            "c_id = :companyId and plant_id = :plantId and wh_id = :warehouseId and lang_id = :languageId and" +
            "bill_freq_id = :billId and is_deleted =0", nativeQuery = true)
    public Long getMonBillDate(@Param("companyId") String companyId,
                                 @Param("plantId") String plantId,
                                 @Param("languageId") String languageId,
                                 @Param("warehouseId") String warehouseId,
                                 @Param("billId") Long billId);
