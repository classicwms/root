...
//            newInventory.(System.currentTimeMillis());
        ...
        try {
            ...
            if (existinginventory != null) {
                ...
                if (INV_QTY >= 0) {
                    ...
                    //inventory2.(Long.valueOf(System.currentTimeMillis() + "" + 8));
...
//            inventory.setPackBarcodes(createdGRLine.getPackBarcodes());
            ...
            //inventory.(Long.valueOf(System.currentTimeMillis() + "" + 1));
     ...
     * @return
     */
    public InventoryV2 createInventoryNonCBMV2(PutAwayLineV2 putAwayLine) {
...
//                    inventory2.(Long.valueOf(System.currentTimeMillis() + "" + 8));
                        InventoryV2 createdInventoryV2 = inventoryV2Repository.save(inventory2);
                        stockMovementService.createInventoryStockMovement(inventory2);
                        log.info("----existinginventory--createdInventoryV2--------> : " + createdInventoryV2);
                    }
                }
            }
...
//            inventory.setPackBarcodes(createdGRLine.getPackBarcodes());
            ...
            InventoryV2 dbInventory = inventoryService.getInventoryForInboundConfirmSameItemCode(putAwayLine.getWarehouseId(), putAwayLine.getCompanyCode(),
                    putAwayLine.getPlantId(), putAwayLine.getLanguageId(), putAwayLine.getItemCode(), putAwayLine.getBarcodeId(), putAwayLine.getConfirmedStorageBin(), putAwayLine.getBusinessPartnerCode());
            log.info("Same BarcodeId and StorageBin in ItemCode " + dbInventory);
            // ThreePL
            ...
            if (dbInventory != null) {
                ...
                //customerName
                if(dbInventory.getThreePLPartnerId() != null) {
                    String customerName = getCustomerName(inventory.getCompanyCodeId(), inventory.getPlantId(),
                            inventory.getLanguageId(), inventory.getWarehouseId(), putAwayLine.getBusinessPartnerCode());
                ...
                Optional.ofNullable(priceListAssignmentRepository.findByCompanyCodeIdAndPlantIdAndWarehouseIdAndPartnerCodeAndDeletionIndicator(
                                putAwayLine.getCompanyCode(), putAwayLine.getPlantId(), putAwayLine.getWarehouseId(), putAwayLine.getBusinessPartnerCode(), 0L))
                        .ifPresentOrElse(priceListAssignment -> {
                            List<PriceList> priceLists = priceListRepository.findByCompanyCodeIdAndPlantIdAndWarehouseIdAndPriceListIdAndServiceTypeIdAndDeletionIndicator(
                                    priceListAssignment.getCompanyCodeId(), priceListAssignment.getPlantId(), priceListAssignment.getWarehouseId(), priceListAssignment.getPriceListId(), 3L, 0L);
                            log.info("PriceList Values {}", priceLists);

                            Double threePLCbm = putAwayLine.getThreePLCbm() != null ? putAwayLine.getThreePLCbm() : 0.0; // Given total CBM
                            ...
                            double totalCbm = putAwayLine.getTotalThreePLCbm() != null ? putAwayLine.getTotalThreePLCbm() : 0;
                            log.info("Finally RATE Value is {}", chargeUnit);
                            inventory.setThreePLPartnerId(putAwayLine.getBusinessPartnerCode());
                            //customerName
                            if(putAwayLine.getBusinessPartnerCode() != null) {
                                String customerName = getCustomerName(inventory.getCompanyCodeId(), inventory.getPlantId(),
                                        inventory.getLanguageId(), inventory.getWarehouseId(), putAwayLine.getBusinessPartnerCode());
                                if (customerName != null) {
                                    inventory.setThreePLPartnerText(customerName);
