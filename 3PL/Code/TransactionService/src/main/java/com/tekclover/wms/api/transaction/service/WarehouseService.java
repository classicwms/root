...
public class WarehouseService extends BaseService {
    ...
    protected static final String MFR_NAME = "3PL";
    ...
    private InboundOrder saveASN(ASN asn) {
        try {
            ...
            for (ASNLine asnLine : asnLines) {
                ...
                apiLine.setManufacturerPartNo(MFR_NAME);    // MFR_PART
...
//				apiHeader.setInboundOrderHeaderId(asnV2Header.getInboundOrderTypeId());
//			} else {
//				apiHeader.setInboundOrderHeaderId(1L);       //Default
//			}
            apiHeader.setInboundOrderTypeId(1L);
            apiHeader.setRefDocumentType("SupplierInvoice");
...
//                apiLine.setSupplierCode(asnLineV2.getSupplierCode());
                if (asnLineV2.getSupplierCode() != null) {                             // PARTNER_CODE
                    apiLine.setSupplierCode(asnLineV2.getSupplierCode());
                } else {
    ...
    private InboundOrderV2 saveSOReturnV2(SaleOrderReturnV2 soReturnV2) {
        try {
            ...
            for (SOReturnLineV2 soReturnLineV2 : salesOrderReturnLinesV2) {
                ...
                apiLine.setManufacturerName(MFR_NAME);        // BRAND_NM
                apiLine.setManufacturerCode(MFR_NAME);
                apiLine.setManufacturerPartNo(MFR_NAME);        // BRND_NM
...
//				apiLine.setOutboundOrderHeaderId(apiHeader.getOutboundOrderHeaderId());
                ...
                if (soHeader.getRequiredDeliveryDate() != null) {
                    if (soHeader.getRequiredDeliveryDate().contains("-")) {
                        // EA_DATE
                        try {
                            Date reqDelDate = new Date();
                            if (soHeader.getRequiredDeliveryDate().length() > 10) {
                                reqDelDate = DateUtils.convertStringToDateWithTime(soHeader.getRequiredDeliveryDate());
                            }
                            if (soHeader.getRequiredDeliveryDate().length() == 10) {
                                reqDelDate = DateUtils.convertStringToDate2(soHeader.getRequiredDeliveryDate());
                            }
                            apiHeader.setRequiredDeliveryDate(reqDelDate);
                        } catch (Exception e) {
                            e.printStackTrace();
                    ...
                    if (soHeader.getRequiredDeliveryDate().contains("/")) {
                        ...
                        try {
                            ZoneId defaultZoneId = ZoneId.systemDefault();
                            String sdate = soHeader.getRequiredDeliveryDate();
                            String firstHalf = sdate.substring(0, sdate.lastIndexOf("/"));
                            String secondHalf = sdate.substring(sdate.lastIndexOf("/") + 1);
                            secondHalf = "/20" + secondHalf;
                            sdate = firstHalf + secondHalf;
...
//				}
                ...
                orderLines.add(apiLine);
                ImBasicData1V2 dbimBasicData1V2 = imBasicData1V2Repository.findByLanguageIdAndCompanyCodeIdAndPlantIdAndWarehouseIdAndItemCodeAndDeletionIndicator(
                        apiHeader.getLanguageId(), apiHeader.getCompanyCode(), apiHeader.getBranchCode(), apiHeader.getWarehouseID(), apiLine.getItemCode(), 0L);
                log.info("dbimBasicData1V2------->" + dbimBasicData1V2);
                if (dbimBasicData1V2 == null) {
                    throw new BadRequestException("The given values CompanyCodeId -"
                            + apiHeader.getCompanyCode() + " PlantId "
                            + apiHeader.getBranchCode() + " WarehouseId "
                            + apiHeader.getWarehouseID() + " languageId "
                            + apiHeader.getLanguageId() + " itemCode "
                            + apiLine.getItemCode() + " doesn't exist");
                }
            }
            apiHeader.setLine(orderLines);
            apiHeader.setOrderProcessedOn(new Date());
...
//			}
    ...
    private StockAdjustment saveStockAdjustment(StockAdjustment stockAdjustment) {
        try {
            ...
            dbStockAdjustment.setManufacturerName(MFR_NAME);        // BRAND_NM
            dbStockAdjustment.setManufacturerCode(MFR_NAME);
