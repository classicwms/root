...
import com.tekclover.wms.api.transaction.model.threepl.dashboard.ListOfTotalVolumes;
import com.tekclover.wms.api.transaction.model.threepl.dashboard.SumOfVolumes;
...
//		log.info("FastSlowMovingDashboard itemData : " + itemData);
     ...
     * @throws java.text.ParseException
     */
    public List<BillingTransactionReportImpl> getBillingTransactionReport(FindBillingTransactionReport findReport) throws java.text.ParseException {
        List<BillingTransactionReportImpl> billingTransactionReportList = new ArrayList<>();
        List<BillingTransactionReportImpl> grCharge = null;
        List<BillingTransactionReportImpl> putawayCharge = null;
        List<BillingTransactionReportImpl> stockMovement = null;
        List<BillingTransactionReportImpl> pickUpCharge = null;
        List<BillingTransactionReportImpl> packingCharge = null;

        log.info("Find BillingTransactionReport Inputs -----> " + findReport);
        if (findReport.getStartCreatedOn() != null && findReport.getEndCreatedOn() != null) {
            Date[] dates = DateUtils.addTimeToDatesForSearch(findReport.getStartCreatedOn(), findReport.getEndCreatedOn());
            findReport.setStartCreatedOn(dates[0]);
            findReport.setEndCreatedOn(dates[1]);
        }

        if(findReport.getServiceTypeId() == null || findReport.getServiceTypeId().contains("1")) {
            grCharge = billingTransactionReportRepository.getGrLine(findReport.getCompanyCodeId(),
                    findReport.getPlantId(), findReport.getLanguageId(), findReport.getWarehouseId(),
                    findReport.getPartnerCode(), findReport.getStartCreatedOn(), findReport.getEndCreatedOn());
            log.info("GrLine Get Records for Reports ......................." + grCharge.size());
        }
        if(findReport.getServiceTypeId() == null || findReport.getServiceTypeId().contains("2")) {
            putawayCharge = billingTransactionReportRepository.getPutAwayLine(findReport.getCompanyCodeId(),
                    findReport.getPlantId(), findReport.getLanguageId(), findReport.getWarehouseId(), findReport.getPartnerCode(),
                    findReport.getStartCreatedOn(), findReport.getEndCreatedOn());
            log.info("PutAwayHeader Get Records for Reports ......................." + putawayCharge.size());
        }
        if(findReport.getServiceTypeId() == null || findReport.getServiceTypeId().contains("3")) {
            stockMovement = billingTransactionReportRepository.getStockMovement(findReport.getCompanyCodeId(), findReport.getPlantId(),
                    findReport.getLanguageId(), findReport.getWarehouseId(), findReport.getPartnerCode(),
                    findReport.getStartCreatedOn(), findReport.getEndCreatedOn());
            log.info("StockMovement Get Records for Reports ......................." + stockMovement.size());
        }
        if(findReport.getServiceTypeId() == null || findReport.getServiceTypeId().contains("4")) {
            pickUpCharge = billingTransactionReportRepository.getPickupLine(findReport.getCompanyCodeId(), findReport.getPlantId(),
                    findReport.getLanguageId(), findReport.getWarehouseId(), findReport.getPartnerCode(),
                    findReport.getStartCreatedOn(), findReport.getEndCreatedOn());
            log.info("PickupLine Get Records for Reports ......................." + pickUpCharge.size());
        }
        if(findReport.getServiceTypeId() == null || findReport.getServiceTypeId().contains("5")) {
            packingCharge = billingTransactionReportRepository.getPackingLine(findReport.getCompanyCodeId(), findReport.getPlantId(),
                    findReport.getLanguageId(), findReport.getWarehouseId(), findReport.getPartnerCode(),
                    findReport.getStartCreatedOn(), findReport.getEndCreatedOn());
            log.info("PackingLine Get Records for Reports ......................." + packingCharge.size());
        }


        if(grCharge != null) {
            billingTransactionReportList.addAll(grCharge);
        }
        if(putawayCharge != null) {
            billingTransactionReportList.addAll(putawayCharge);
        }
        if(stockMovement != null) {
            billingTransactionReportList.addAll(stockMovement);
        }
        if(pickUpCharge != null) {
            billingTransactionReportList.addAll(pickUpCharge);
        }
        if(packingCharge != null) {
            billingTransactionReportList.addAll(packingCharge);
        }
        return billingTransactionReportList;
    }

    //Find Dash Board

    public List<DashBoard> findDashBoard(FindDashBoard findDashBoard) throws java.text.ParseException {

        List<DashBoard> dashBoardList = new ArrayList<>();

        if (findDashBoard.getPartnerId().isEmpty()) {
            throw new BadRequestException("Partner ID is mandatory and cannot be empty.");
        }
        if (findDashBoard.getFromDate() != null && findDashBoard.getToDate() != null) {
            Date[] dates = DateUtils.addTimeToDatesForSearch(findDashBoard.getFromDate(), findDashBoard.getToDate());
            findDashBoard.setFromDate(dates[0]);
            findDashBoard.setToDate(dates[1]);
        }

        IKeyValuePair sumOfVolumes = inventoryRepository.getSumOfVolumes(findDashBoard.getCompanyCodeId(), findDashBoard.getPlantId(), findDashBoard.getLanguageId()
                , findDashBoard.getWarehouseId(), findDashBoard.getPartnerId(), findDashBoard.getFromDate(), findDashBoard.getToDate());

        DashBoard newDashBoard = new DashBoard();
        List<ListOfTotalVolumes> listOfTotalVolumes = new ArrayList<>();

        // sum of volumes
        log.info("--------------Sum of Volumes--------------");
        SumOfVolumes volumes = new SumOfVolumes();
        BeanUtils.copyProperties(sumOfVolumes, volumes, CommonUtils.getNullPropertyNames(sumOfVolumes));
        log.info("Sum of volumes : {}" + volumes);

        List<IKeyValuePair> listOfVolume = inventoryRepository.getVolumes(findDashBoard.getCompanyCodeId(), findDashBoard.getPlantId(), findDashBoard.getLanguageId(),
                findDashBoard.getWarehouseId(), findDashBoard.getPartnerId(), findDashBoard.getFromDate(), findDashBoard.getToDate());

        // list of total volumes
        log.info("-------------List of Total Volumes------------");
        if (!listOfVolume.isEmpty()) {
            for (IKeyValuePair listOfVolumes : listOfVolume) {

                ListOfTotalVolumes newListOfVolumes = new ListOfTotalVolumes();

                BeanUtils.copyProperties(listOfVolumes, newListOfVolumes, CommonUtils.getNullPropertyNames(listOfVolumes));
                listOfTotalVolumes.add(newListOfVolumes);
            }
            //      log.info("List of Total Volumes : "+ listOfVolume);
        }
        newDashBoard.setListOfTotalVolumes(listOfTotalVolumes);
        newDashBoard.setSumOfVolumes(volumes);
