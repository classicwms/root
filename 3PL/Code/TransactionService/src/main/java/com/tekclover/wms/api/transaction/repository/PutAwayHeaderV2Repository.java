...
public interface PutAwayHeaderV2Repository extends JpaRepository<PutAwayHeaderV2, Long>,
    ...
    @Query(value = "select max(inv_id) inventoryId into #inv from tblinventory \n"
            ...
            + "TPL_CBM threePLCbm, \n"
            + "TPL_UOM threePLUom, \n"
            + "TPL_LENGTH threePLLength, \n"
            + "TPL_WIDTH threePLHeight, \n"
            + "TPL_HEIGHT threePLWidth \n"
    ...
    @Query(value = "select SUM(TOTAL_TPL_CBM) totalCbm, sum(RATE) totalRate, min(currency) currency from tblputawayline where c_id = :companyId and plant_id = :plantId " +
            "AND lang_id = :languageId and wh_id = :warehouseId " +
            "AND partner_code = :partnerCode and is_deleted = 0 And (PROFORMA_INVOICE_NO is null or PROFORMA_INVOICE_NO = 0) " +
