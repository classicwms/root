...
//====================================================TNG=====================================================
    ...
    public OrderResponse purchaseOrder(PurchaseOrder purchaseOrder) {
        try {
...
//			headers.add("Authorization", "Basic " + encodedCredentials);
            ...
            UriComponentsBuilder builder =
                    UriComponentsBuilder.fromHttpUrl(getTNGUrl() + "CreatePurchaseOrder");

            HttpEntity<?> entity = new HttpEntity<>(purchaseOrder, headers);
            ResponseEntity<OrderResponse> result =
                    getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, OrderResponse.class);

            log.info("---post purchaseOrder -----StatusCode----->: " + result.getStatusCodeValue());
            log.info("---post purchaseOrder -----ResponseBody--->: " + result.getBody());
    ...
    public OrderResponse updateShipmentOrder(UpdateShipmentOrder updateShipmentOrder) {
        try {
//			String credentials = propertiesConfig.getTngSecretKey() + ":" + propertiesConfig.getTngSecretValue();
//			String encodedCredentials = new String(Base64.encodeBase64(credentials.getBytes()));
...
//			headers.add("Authorization", "Basic " + encodedCredentials);
            ...
            UriComponentsBuilder builder =
                    UriComponentsBuilder.fromHttpUrl(getTNGUrl() + "UpdateShipmentOrder");

            HttpEntity<?> entity = new HttpEntity<>(updateShipmentOrder, headers);
            ResponseEntity<OrderResponse> result =
                    getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, OrderResponse.class);
    ...
    public CancelShipmentOrderResponse cancelShipmentOrder(CancelShipmentOrder cancelShipmentOrder) {
        try {
//			String credentials = propertiesConfig.getTngSecretKey() + ":" + propertiesConfig.getTngSecretValue();
//			String encodedCredentials = new String(Base64.encodeBase64(credentials.getBytes()));
...
//			headers.add("Authorization", "Basic " + encodedCredentials);
            ...
            UriComponentsBuilder builder =
                    UriComponentsBuilder.fromHttpUrl(getTNGUrl() + "CancelShipmentOrder");

            HttpEntity<?> entity = new HttpEntity<>(cancelShipmentOrder, headers);
            ResponseEntity<CancelShipmentOrderResponse> result =
                    getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, CancelShipmentOrderResponse.class);

            log.info("---post cancelShipmentOrder -----StatusCode----->: " + result.getStatusCodeValue());
            log.info("---post cancelShipmentOrder -----ResponseBody--->: " + result.getBody());
    ...
    //Fetch Stock
    public List<FetchStock> fetchStock(Fetch fetch) {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
            headers.set(propertiesConfig.getTngSecretKey(), propertiesConfig.getTngSecretValue());
            ...
            ResponseEntity<FetchStockResponse> result =
                    getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, FetchStockResponse.class);

            log.info("---FetchStock API StatusCode----->: {}", result.getStatusCodeValue());
            log.info("---FetchStock API ResponseBody--->: {}", result.getBody());

            FetchStockResponse response = result.getBody();
