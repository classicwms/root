...
//                                    dbGrLine.getCompanyCode(), dbGrLine.getPlantId(), dbGrLine.getWarehouseId(), dbGrLine.getBusinessPartnerCode(), 0L);
//                            log.info("PriceListAssignment Values Get PriceListId {} ", priceListAssignment.getPriceListId());
                                ...
                                Optional.ofNullable(priceListAssignmentRepository.findByCompanyCodeIdAndPlantIdAndWarehouseIdAndPartnerCodeAndDeletionIndicator(
                                        ...
                                        .ifPresentOrElse(priceListAssignment -> {
                                            List<PriceList> priceLists = priceListRepository.findByCompanyCodeIdAndPlantIdAndWarehouseIdAndPriceListIdAndServiceTypeIdAndDeletionIndicator(
                                                    priceListAssignment.getCompanyCodeId(), priceListAssignment.getPlantId(), priceListAssignment.getWarehouseId(), priceListAssignment.getPriceListId(), 1L, 0L);
                                            log.info("PriceList Values {}", priceLists);

                                            Double threePLCbm = dbGrLine.getThreePLCbm(); // Given total CBM
...
//                                                currency.append(dbPrice.getReferenceField4());
                                                ...
                                                // Calculate applicable volume for this range
//                                if (remainingCBM > chargeFrom) {
//                                    Double applicableVolume = Math.min(remainingCBM, chargeTo) - chargeFrom;
//
//                                    if (applicableVolume > 0) {
//                                        totalCbm += applicableVolume; // Sum applicable volume
//                                        chargeUnit = chargePerUnit;   // Store the latest chargePerUnit
//                                        remainingCBM -= applicableVolume; // Reduce remaining CBM
//                                    }
//                                }

                                                // Stop processing if all CBM is covered
                                            ...
                                            double totalThreePLCbm = dbGrLine.getTotalThreePLCbm() != null ? dbGrLine.getTotalThreePLCbm() : 0;
                                            log.info("Finally Rate Value is {}", chargeUnit);
                                            dbGrLine.setRate(chargeUnit);
                                            dbGrLine.setCurrency(String.valueOf(currency));
                                            dbGrLine.setReferenceField5(String.valueOf(chargeUnit));
                                            dbGrLine.setThreePLBillStatus("GR");
                                            dbGrLine.setTotalRate(totalThreePLCbm * chargeUnit);
                                            dbGrLine.setReferenceField1(String.valueOf(currency));
                                            dbGrLine.setReferenceField4(String.valueOf(priceDescription));
                                        }, () -> log.warn("PriceListAssignment not found for CompanyCode {}, PlantId {}, WarehouseId {}, PartnerCode {}",
                                                dbGrLine.getCompanyCode(), dbGrLine.getPlantId(), dbGrLine.getWarehouseId(), dbGrLine.getBusinessPartnerCode()));
                            } catch (Exception e) {
                                log.info("ThreePL Logic Error Throw " +  e.getMessage());
                            }
                        }
            ...
            if (createdGRLine != null) {
                ...
                try {
                    ...
                    if (createdGRLine.getThreePLCbm() != null) {
                        Double cubicRootValue = Math.cbrt(createdGRLine.getThreePLCbm());

                        putAwayHeader.setThreePLLength(cubicRootValue);
                        putAwayHeader.setThreePLWidth(cubicRootValue);
                        putAwayHeader.setThreePLHeight(cubicRootValue);
                    }

                    Optional<PriceListAssignment> priceListAssignmentOpt = Optional.ofNullable(
                            priceListAssignmentRepository.findByCompanyCodeIdAndPlantIdAndWarehouseIdAndPartnerCodeAndDeletionIndicator(
                                    createdGRLine.getCompanyCode(), createdGRLine.getPlantId(), createdGRLine.getWarehouseId(), createdGRLine.getBusinessPartnerCode(), 0L
                            ));
                    if (priceListAssignmentOpt.isPresent()) {
                        PriceListAssignment priceListAssignment = priceListAssignmentOpt.get();
                        List<PriceList> priceLists = priceListRepository.findByCompanyCodeIdAndPlantIdAndWarehouseIdAndPriceListIdAndServiceTypeIdAndDeletionIndicator(
                                priceListAssignment.getCompanyCodeId(), priceListAssignment.getPlantId(), priceListAssignment.getWarehouseId(), priceListAssignment.getPriceListId(), 2L, 0L);
                        log.info("PriceList Values {}", priceLists);

                        Double threePLCbm = createdGRLine.getThreePLCbm(); // Given total CBM
                        String currency = new String();
                        StringBuilder priceDescription = new StringBuilder();
                        double chargeUnit = 0.0;
                        ...
                        for (PriceList dbPrice : priceLists) {
                            double chargeFrom = Optional.ofNullable(dbPrice.getChargeRangeFrom()).orElse(0.0);
                            double chargeTo = Optional.ofNullable(dbPrice.getChargeRangeTo()).orElse(0.0);
                            double chargePerUnit = Optional.ofNullable(dbPrice.getPricePerChargeUnit()).orElse(0.0);

                            currency = dbPrice.getReferenceField4();
                            priceDescription.append(Optional.ofNullable(dbPrice.getReferenceField1()).orElse(""));
                        ...
                        double totalCbm = createdGRLine.getTotalThreePLCbm() != null ? createdGRLine.getTotalThreePLCbm() : 0;
                        log.info("Finally ThreePLUom Value is {}", chargeUnit);
                        putAwayHeader.setRate(chargeUnit);
                        putAwayHeader.setBusinessPartnerCode(createdGRLine.getBusinessPartnerCode());
                        log.info("Currency------>" + currency);
                        putAwayHeader.setCurrency(String.valueOf(currency));
                        putAwayHeader.setReferenceField5(String.valueOf(chargeUnit));
                        putAwayHeader.setThreePLBillStatus("Putaway Charges");
                        putAwayHeader.setTotalRate(totalCbm * chargeUnit);
                        putAwayHeader.setReferenceField1(String.valueOf(currency));
                        putAwayHeader.setReferenceField4(String.valueOf(priceDescription));
                    } else {
                        log.warn("PriceListAssignment not found for CompanyCode {}, PlantId {}, WarehouseId {}, PartnerCode {}",
                                createdGRLine.getCompanyCode(), createdGRLine.getPlantId(), createdGRLine.getWarehouseId(), createdGRLine.getBusinessPartnerCode());
                    }
                } catch (Exception e) {
                    log.info("PutAwayHeader Create ThreePL Logic Exception Throw -----> " + e.getMessage());
                }
                log.info("ThreePL Logic completed -----------------------------------------> ");
                //-----------------PROP_ST_BIN---------------------------------------------
...
//        }
     ...
     * @param authToken
     */
    public void updateEmptyStorageBinStatus(String storageBin, String companyCode, String plantId, String
            languageId,
                                            String warehouseId, String loginUserId, String authToken) {
     ...
     * @param authToken
     */
    public Double emptyCbmPerQtyStorageBinUpdate(StorageBinV2 storageBin, Double cbm, String loginUserID, String
            authToken) {
     ...
     * @return
     */
    private StorageBinV2[] getStorageBinV2(Long binClassId, List<String> stBins, String companyCodeId, String
            plantId,
    ...
    private InventoryV2 createInventoryNonCBMV2(GrLineV2 createdGRLine) {
        ...
        try {
            ...
            if (dbInventory != null) {
                ...
                //customerName
                if(createdGRLine.getBusinessPartnerCode() != null) {
                    String customerName = getCustomerName(inventory.getCompanyCodeId(), inventory.getPlantId(),
                            inventory.getLanguageId(), inventory.getWarehouseId(), createdGRLine.getBusinessPartnerCode());
...
//                stockMovementService.createInventoryStockMovement(createdinventory);
            }
            ...
            if (dbInventory == null) {
                ...
                //customerName
                if(preInboundHeader.get(0).getBusinessPartnerCode() != null) {
                    String customerName = getCustomerName(inventory.getCompanyCodeId(), inventory.getPlantId(),
                            inventory.getLanguageId(), inventory.getWarehouseId(), preInboundHeader.get(0).getBusinessPartnerCode());
                    if (customerName != null) {
                        inventory.setThreePLPartnerText(customerName);
                    }
                }
...
//            inventory.setPackBarcodes(createdGRLine.getPackBarcodes());
                ...
                inventory.setReferenceDocumentNo(createdGRLine.getRefDocNumber());
                inventory.setReferenceOrderNo(createdGRLine.getRefDocNumber());
                inventory.setThreePLPartnerId(createdGRLine.getBusinessPartnerCode());
                //customerName
                if(createdGRLine.getBusinessPartnerCode() != null) {
                    String customerName = getCustomerName(inventory.getCompanyCodeId(), inventory.getPlantId(),
                            inventory.getLanguageId(), inventory.getWarehouseId(), createdGRLine.getBusinessPartnerCode());
                ...
                inventory.setUpdatedOn(new Date());
//                 // inventory.Long.valueOf(System.currentTimeMillis() + "" + 8));
                createdinventory = inventoryV2Repository.save(inventory);
//                stockMovementService.createInventoryStockMovement(createdinventory);
            }
...
//                inventory.Long.valueOf(System.currentTimeMillis() + "" + 8));
    ...
    public void deleteGrLineV2(String companyCodeId, String plantId, String languageId,
        ...
        if (grLine != null) {
            ...
            stockMovementService.deleteGrLineDelete(companyCodeId, plantId, languageId, warehouseId, itemCode, refDocNumber, loginUserID);
        } else {
            // Exception Log
    ...
    public List<GrLineV2> deleteGrLineV2(String companyCode, String plantId, String languageId,
                                         String warehouseId, String refDocNumber, String preInboundNo, String loginUserID) throws
    ...
    //============================================GrLine_ExceptionLog==================================================
    private void createGrLineLog(String languageId, String companyCode, String plantId, String warehouseId, String
            refDocNumber,
