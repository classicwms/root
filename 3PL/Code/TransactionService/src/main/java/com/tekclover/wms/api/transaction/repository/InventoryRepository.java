...
//	Stream<InventoryImpl> findInventory (
    ...
    //    @Query(value = "select sum(total_volumes) as sumOfTotalVolumes ,sum(occupied_volumes) as sumOfOccupiedVolumes,sum(remaining_volumes) as sumOfRemainingVolumes from tblinventory " +
//            "where (coalesce(:languageId , null) is null or (lang_id IN (:languageId))) and" +
...
//            "and tpl_partner_id =:partnerId and iu_ctd_on between coalesce(convert(varchar(255), :fromDate), null) and coalesce(convert(varchar(255), :toDate), null) and" +
//            " is_deleted=0 and bin_cl_id = 3", nativeQuery = true)
//    IKeyValuePair getSumOfVolumes(@Param("companyCodeId") String companyCodeId,
...
//                                  @Param("toDate")Date toDate);
    ...
    @Query(value = "select sum(total_volumes) as sumOfTotalVolumes ,sum(occupied_volumes) as sumOfOccupiedVolumes,sum(total_volumes - occupied_volumes) as sumOfRemainingVolumes from tblinventory " +
            "where (coalesce(:languageId , null) is null or (lang_id IN (:languageId))) and" +
            "(coalesce(:companyCodeId , null) is null or (c_id IN (:companyCodeId))) and " +
            "(coalesce(:plantId , null) is null or (plant_id IN (:plantId))) and " +
            "(coalesce(:warehouseId , null) is null or (wh_id IN (:warehouseId)))" +
            "and tpl_partner_id =:partnerId and iu_ctd_on between coalesce(convert(varchar(255), :fromDate), null) and coalesce(convert(varchar(255), :toDate), null) and" +
            " is_deleted=0 and bin_cl_id = 1", nativeQuery = true)
    IKeyValuePair getSumOfVolumes(@Param("companyCodeId") String companyCodeId,
                                  @Param("plantId") String plantId,
                                  @Param("languageId") String languageId,
                                  @Param("warehouseId") String warehouseId,
    ...
    @Query(value = "select inv_id as inventoryId , tpl_partner_id as threePLPartnerId , st_bin as storageBin, occupied_volumes as occupiedVolumes, (total_volumes - occupied_volumes) as remainingVolumes, total_volumes as totalVolumes, bin_cl_id as binClassId from tblinventory " +
            "where (coalesce(:languageId , null) is null or (lang_id IN (:languageId))) and" +
            "(coalesce(:companyCodeId , null) is null or (c_id IN (:companyCodeId))) and " +
            "(coalesce(:plantId , null) is null or (plant_id IN (:plantId))) and " +
            "(coalesce(:warehouseId , null) is null or (wh_id IN (:warehouseId)))" +
            "and tpl_partner_id =:partnerId and iu_ctd_on between coalesce(convert(varchar(255), :fromDate), null) and coalesce(convert(varchar(255), :toDate), null) and" +
            " is_deleted=0 and bin_cl_id = 1",nativeQuery = true)
    public List<IKeyValuePair> getVolumes(@Param("companyCodeId") String companyCodeId,
                                          @Param("plantId") String plantId,
                                          @Param("languageId") String languageId,
                                          @Param("warehouseId") String warehouseId,
                                          @Param("partnerId") String partnerId,
