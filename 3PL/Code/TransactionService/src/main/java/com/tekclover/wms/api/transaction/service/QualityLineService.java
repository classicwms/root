...
public class QualityLineService extends BaseService {
    ...
    @Autowired
    private PackingHeaderService packingHeaderService;
        ...
        try {
            ...
            for (AddQualityLine newQualityLine : newQualityLines) {
                ...
                if (existingQualityLine == null) {
                    ...
                    String packInd = qualityLineRepository.getPackInd(createdQualityLine.getPartnerCode(), createdQualityLine.getWarehouseId(),
                            createdQualityLine.getCompanyCodeId(), createdQualityLine.getRefDocNumber(), createdQualityLine.getPlantId(),
                            createdQualityLine.getPreOutboundNo(), createdQualityLine.getQualityInspectionNo());
                    if (packInd.equalsIgnoreCase("0")) {
                        packingHeaderService.createPackingQualityHeader(createdQualityLine);
                    }
...
//                    }
            ...
            for (AddQualityLineV2 dbQualityLine : newQualityLines) {
                ...
                if (PARTNER_CODE == null || PARTNER_CODE == 0) {
                    ...
                    InventoryV2 inventory = inventoryService.getInventoryV2(dbQualityLine.getCompanyCodeId(),
                            dbQualityLine.getPlantId(), dbQualityLine.getLanguageId(), dbQualityLine.getWarehouseId(),
                            dbQualityLine.getPickPackBarCode(), dbQualityLine.getItemCode());

                    Double TPL_CBM_PER_QTY = inventory.getThreePLCbmPerQty() != null ? inventory.getThreePLCbmPerQty() : 0.0;
                    Double QC_QTY = dbQualityLine.getQualityQty() != null ? dbQualityLine.getQualityQty() : 0.0;

                    if (TPL_CBM_PER_QTY != 0.0) {
                        if (QC_QTY != 0.0) {
                            packingHeader.setPackCbm(TPL_CBM_PER_QTY * QC_QTY);
                        }
                    }
                    packingHeader.setPackingNo(packingNo);
                    packingHeader.setPackBarcode(dbQualityLine.getPickPackBarCode());
                    packingHeader.setOutboundOrderTypeId(dbQualityLine.getOutboundOrderTypeId());
                    packingHeader.setQualityConfirmQty(dbQualityLine.getQualityQty());
                    packingHeader.setQualityConfirmUom(dbQualityLine.getQualityConfirmUom());
                    packingHeader.setStatusId(66L);
                    statusDescription = stagingLineV2Repository.getStatusDescription(66L, dbQualityLine.getLanguageId());
                    packingHeader.setStatusDescription(statusDescription);
                    packingHeader.setDeletionIndicator(0L);
                    packingHeader.setPackConfirmedBy(dbQualityLine.getQualityConfirmedBy());
                    packingHeader.setPackConfirmedOn(new Date());
