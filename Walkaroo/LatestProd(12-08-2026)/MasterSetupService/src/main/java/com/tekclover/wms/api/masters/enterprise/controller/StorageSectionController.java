...
public class StorageSectionController {
    ...
    public List<StorageSection> findStorageSection(@RequestBody SearchStorageSection searchStorageSection)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchStorageSection.getPlantId(), searchStorageSection.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postStorageSection(@Valid @RequestBody AddStorageSection newStorageSection,
        ...
        try {
            String currentDB = baseService.getDataBase(newStorageSection.getPlantId(), newStorageSection.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
