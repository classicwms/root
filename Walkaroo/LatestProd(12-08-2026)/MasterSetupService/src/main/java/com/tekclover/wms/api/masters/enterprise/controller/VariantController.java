...
public class VariantController {
    ...
    public List<Variant> findVariant(@RequestBody SearchVariant searchVariant)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchVariant.getPlantId(), searchVariant.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postVariant(@Valid @RequestBody List<AddVariant> newVariant, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newVariant.get(0).getPlantId(), newVariant.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
