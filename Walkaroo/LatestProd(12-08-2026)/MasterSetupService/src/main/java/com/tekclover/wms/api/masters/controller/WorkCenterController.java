...
public class WorkCenterController {
    ...
    public List<WorkCenter> findWorkCenter(@RequestBody SearchWorkCenter searchWorkCenter)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchWorkCenter.getPlantId().get(0), searchWorkCenter.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postWorkCenter(@Valid @RequestBody AddWorkCenter newWorkCenter, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newWorkCenter.getPlantId(), newWorkCenter.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
