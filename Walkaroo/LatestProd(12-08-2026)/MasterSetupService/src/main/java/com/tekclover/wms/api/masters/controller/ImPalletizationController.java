...
public class ImPalletizationController {
    ...
    public List<ImPalletization> findImPalletization(@RequestBody SearchImPalletization searchImPalletization)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchImPalletization.getPlantId().get(0), searchImPalletization.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postImPalletization(@Valid @RequestBody List<AddImPalletization> newImPalletization,
        ...
        try {
            String currentDB = baseService.getDataBase(newImPalletization.get(0).getPlantId(), newImPalletization.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
