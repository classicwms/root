...
public class ImVariantController {
    ...
    public List<ImVariant> findImVariant(@RequestBody SearchImVariant searchImVariant)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchImVariant.getPlantId().get(0), searchImVariant.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postImVariant(@Valid @RequestBody List<AddImVariant> newImVariant, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newImVariant.get(0).getPlantId(), newImVariant.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
