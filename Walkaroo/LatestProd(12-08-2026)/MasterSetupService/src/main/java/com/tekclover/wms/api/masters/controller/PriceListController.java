...
public class PriceListController {
    ...
    public ResponseEntity<?> postPriceListId(@Valid @RequestBody AddPriceList newPriceListId,
        ...
        try {
            String currentDB = baseService.getDataBase(newPriceListId.getPlantId(), newPriceListId.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> findPriceList(@Valid @RequestBody FindPriceList findPriceList) throws Exception {
        try {
            String currentDB = baseService.getDataBase(findPriceList.getPlantId(), findPriceList.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
