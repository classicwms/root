...
public class ItemTypeController {
    ...
    public List<ItemType> findItemType(@RequestBody SearchItemType searchItemType)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchItemType.getPlantId(), searchItemType.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postItemType(@Valid @RequestBody AddItemType newItemType, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newItemType.getPlantId(), newItemType.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
