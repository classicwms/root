...
public class StorageTypeController {
    ...
    public List<StorageType> findStorageType(@RequestBody SearchStorageType searchStorageType)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchStorageType.getPlantId(), searchStorageType.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postStorageType(@Valid @RequestBody AddStorageType newStorageType, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newStorageType.getPlantId(), newStorageType.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
