...
public class VehicleController {
    ...
    public List<Vehicle> findVehicle(@RequestBody SearchVehicle searchVehicle)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchVehicle.getPlantId().get(0), searchVehicle.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postVehicle(@Valid @RequestBody AddVehicle newVehicle, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newVehicle.getPlantId(), newVehicle.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
