...
import com.tekclover.wms.api.inbound.transaction.model.cyclecount.periodic.*;
import com.tekclover.wms.api.inbound.transaction.model.cyclecount.periodic.v2.PeriodicHeaderEntityV2;
import com.tekclover.wms.api.inbound.transaction.model.cyclecount.periodic.v2.PeriodicHeaderV2;
import com.tekclover.wms.api.inbound.transaction.model.cyclecount.periodic.v2.SearchPeriodicHeaderV2;
...
import com.tekclover.wms.api.inbound.transaction.service.PeriodicHeaderService;
...
public class PeriodicHeaderController {
    ...
    public ResponseEntity<?> postPeriodicHeader(@Valid @RequestBody AddPeriodicHeader newPeriodicHeader,
        ...
        try {
            String currentDB = baseService.getDataBase(newPeriodicHeader.getPlantId(),newPeriodicHeader.getWarehouseId());
    ...
    public ResponseEntity<?> patchPeriodicHeader(@PathVariable String cycleCountNo, @RequestParam String warehouseId,
                                                 ...
                                                 @RequestParam String loginUserID) throws IllegalAccessException, InvocationTargetException {
        try {
            String currentDB = baseService.getDataBase(updatePeriodicHeader.getPlantId(),updatePeriodicHeader.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    @ApiOperation(response = PeriodicHeaderEntityV2.class, value = "Get all PeriodicHeaderV2 details")
    // label for swagger
    @GetMapping("/v2")
