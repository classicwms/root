...
import com.tekclover.wms.api.inbound.transaction.service.DashboardReportService;
...
public class    ReportsController {
    ...
    public ResponseEntity<?> findMobileDashBoard(@Valid @RequestBody FindMobileDashBoard findMobileDashBoard) throws Exception {
        ...
        try {
            String currentDB = baseService.getDataBase(findMobileDashBoard.getPlantId().get(0),findMobileDashBoard.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
...
//	}
    ...
    public ResponseEntity<?> getAllStockReport(@RequestParam List<String> languageId,
        ...
        try {
            String currentDB = baseService.getDataBase(plantId.get(0),warehouseId.get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> getAllStockReportV2(@Valid @RequestBody SearchStockReport searchStockReport) {
        try {
            String currentDB = baseService.getDataBase(searchStockReport.getPlantId().get(0),searchStockReport.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> getAllStockReportV2SP(@Valid @RequestBody SearchStockReportInput searchStockReport) {
        try {
            String currentDB = baseService.getDataBase(searchStockReport.getPlantId(),searchStockReport.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
...
//	}
    ...
    public ResponseEntity<?> getTransactionHistoryReport(@RequestBody FindImBasicData1 searchImBasicData1) throws java.text.ParseException {
        try {
            String currentDB = baseService.getDataBase(searchImBasicData1.getPlantId(),searchImBasicData1.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> getStorageBinDashBoard(@RequestBody StorageBinDashBoardInput storageBinDashBoardInput) throws Exception {
        try {
            String currentDB = baseService.getDataBase(storageBinDashBoardInput.getPlantId(),storageBinDashBoardInput.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> getBinningProductivityReport(@RequestBody SearchBinningProductivityReport searchBinningProductivityReport) throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchBinningProductivityReport.getPlantId().get(0),searchBinningProductivityReport.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> getNewReport(@RequestBody TransactionReport findReport) throws Exception {
        try {
            String currentDB = baseService.getDataBase(findReport.getPlantId().get(0),findReport.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    //---------------------------------------------Inbound Reversal------------------------------------------------------------
    @ApiOperation(response = BinOccupancyDashboardResponse.class, value = "Capacity DashBoard")    // label for swagger
    @GetMapping("/bin/occupancy/dashboard")
    public ResponseEntity<?> capacityDashBoard() {
            BinOccupancyDashboardResponse occupancyRequest =  dashboardReportService.capacityDashBoard();
            return new ResponseEntity<>(occupancyRequest, HttpStatus.OK);
    }
