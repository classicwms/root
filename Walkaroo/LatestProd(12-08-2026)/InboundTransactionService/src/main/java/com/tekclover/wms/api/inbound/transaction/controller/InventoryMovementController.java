...
public class InventoryMovementController {
    ...
    public Stream<InventoryMovement> findInventoryMovement(@RequestBody SearchInventoryMovement searchInventoryMovement)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchInventoryMovement.getPlantId().get(0),searchInventoryMovement.getWarehouseId().get(0));
    ...
    public ResponseEntity<?> postInventoryMovement(@Valid @RequestBody AddInventoryMovement newInventoryMovement, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newInventoryMovement.getPlantId(),newInventoryMovement.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> patchInventoryMovement(@PathVariable Long movementType, @RequestParam String warehouseId,
            ...
            throws IllegalAccessException, InvocationTargetException {
        try {
            String currentDB = baseService.getDataBase(updateInventoryMovement.getPlantId(),updateInventoryMovement.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
