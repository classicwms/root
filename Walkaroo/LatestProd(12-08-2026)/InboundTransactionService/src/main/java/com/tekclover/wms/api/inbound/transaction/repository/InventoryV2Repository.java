...
import com.tekclover.wms.api.inbound.transaction.model.dto.OperationConsumptionImpl;
...
//            "group by itm_code,barcode_id,mfr_name,pack_barcode,stck_typ_id,st_bin,plant_id,wh_id,c_id,lang_id \n" +
  ...
  //------Added-by--Murugavel--on--10/03---for---InventoryTrans-----------
    ...
    @Query(value = "SELECT * from tblinventory " +
            "WHERE inv_id in (select max(inv_id) from tblinventory where is_deleted = 0 group by itm_code,barcode_id,mfr_name,pack_barcode,stck_typ_id,st_bin,plant_id,wh_id,c_id,lang_id) and " +
            "c_id in (:companyCodeId) and " +
            "plant_id in (:plantId) and " +
            "bin_cl_id in (1,3) and " +
            "ref_field_4 > 0 and " +
            "wh_id in (:warehouseId) and  is_deleted = 0 and " +
            "(COALESCE(:itemCode, null) IS NULL OR (ITM_CODE IN (:itemCode))) and \n" +
            "(COALESCE(:barcodeId, null) IS NULL OR (BARCODE_ID IN (:barcodeId))) ", nativeQuery = true)
    List<InventoryV2> getItemBarcodeInvQty(@Param("companyCodeId") List<String> companyCodeId,
                                                       @Param("plantId") List<String> plantId,
                                                       @Param("warehouseId") List<String> warehouseId,
                                                       @Param("itemCode") List<String> itemCode,
                                                       @Param("barcodeId") List<String> barcodeId);
    ...
    @Query(value = "SELECT count(*) FROM tblinventory\n" +
            ...
            "\t and wh_id = :warehouseId and REF_DOC_NO = :refDocNo and bin_cl_id in (1, 3) ", nativeQuery = true)
    Long getInventoryBin3Count(@Param("barcodeId") String barcodeId,
                               @Param("companyId") String companyId,
                               @Param("plantId") String plantId,
                               @Param("warehouseId") String warehouseId,
                               @Param("refDocNo") String refDocNumber);
}
