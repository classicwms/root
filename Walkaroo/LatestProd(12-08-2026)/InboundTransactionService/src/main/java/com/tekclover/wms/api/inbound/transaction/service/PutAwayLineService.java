...
//                log.info("StagingLine: " + dbStagingLineEntity);
                if (dbStagingLineEntity != null) {
                    ...
                    newPutAwayLine.setReferenceField10(dbStagingLineEntity.getVehicleNumber());
                }
...
//            UnexpectedRollbackException.class}, maxAttempts = 2, backoff = @Backoff(delay = 2000))
    ...
    // Inbound Confirm
    public void inboundConfirmValidationInKafka (String companyCode, String plantId, String languageId, String warehouseId,
                                           String refDocNumber, String preInboundNo, String loginUserID) {
        //Checking sum of ordered qty with sum of putaway cnf qty excluding reversal status
        Long confirmedLines = inboundHeaderV2Repository.checkAutoConfirmStatus(companyCode, plantId, languageId, warehouseId, refDocNumber, 22L);
        log.info("AutoConfirm Inbound: " + confirmedLines);
        if (confirmedLines == 1) {
            log.info("Initiate Automatic Inbound Confirmation------> " + refDocNumber + "---> " + preInboundNo);
            inboundHeaderService.inboundConfirmStatusUpdate(companyCode, plantId, languageId, warehouseId, preInboundNo, refDocNumber, loginUserID);
        }
    }
    ...
    public List<PutAwayLineV2> putAwayLineConfirmValidation(List<PutAwayLineV2> newPutAwayLines, String loginUserID) {
        ...
        try {
            ...
            for (PutAwayLineV2 newPutAwayLine : newPutAwayLines) {
                ...
                boolean existingPutAwayLine = putAwayLineV2Repository
                        .existsByCompanyCodeAndPlantIdAndLanguageIdAndWarehouseIdAndBarcodeIdAndRefDocNumberAndDeletionIndicator(
                                newPutAwayLine.getCompanyCode(), newPutAwayLine.getPlantId(), newPutAwayLine.getLanguageId(),
                                newPutAwayLine.getWarehouseId(), newPutAwayLine.getBarcodeId(), newPutAwayLine.getRefDocNumber(), 0L);
                log.info("Existing putawayline already created : " + existingPutAwayLine);
            ...
            if(!createdPutAwayLines.isEmpty()) {
                try {
                    updatePutAwayHeader(companyCode, plantId, languageId, warehouseId, createdPutAwayLines);
                } catch (Exception e) {
                    log.error("Error updating PutAwayHeader status for barcodes: {}", createdPutAwayLines, e);
                }
     ...
     * Kafka
     *
     * @param newPutAwayLines newPutAwayLine
     * @param loginUserID loginUserID
     * @return return PutAwayLine
     */
    public void putAwayLineConfirmKafka(@Valid List<PutAwayLineV2> newPutAwayLines, String loginUserID) {
        List<PutAwayLineV2> createdPutAwayLines = new ArrayList<>();
        log.info("newPutAwayLines to confirm : " + newPutAwayLines);
        ...
        try {
            ...
            for (PutAwayLineV2 newPutAwayLine : newPutAwayLines) {
                ...
                if (!existingPutAwayLine) {
                    ...
                    // Create Inventory
                    try {
                        InventoryV2 newInventoryBinClassId1 = inventoryService.createInventoryInPutAwayLine(dbPutAwayLine, loginUserID);
...
//                    }
            ...
            if(!createdPutAwayLines.isEmpty()){
                ...
                log.info("PutAwayLine Saved Value size ---> {}, --> And --> Values {} ", createdPutAwayLines.size(), createdPutAwayLines);
            }
            ...
            if(refDocNumbers !=null && !refDocNumbers.isEmpty()) {
                ...
                for (PutAwayLineConfirm docNumber : refDocNumbers) {
                    ...
                    try {
                        ...
                        putAwayLineV2Repository.updateReceivedLinesCount(
                                receivedLines, companyCode, plantId, languageId, warehouseId,
                                docNumber.getRefDocNumber(), docNumber.getPreInboundNo());

                        log.info("InboundHeader received lines count updated : {} - {}, Count : {}", docNumber.getRefDocNumber(), docNumber.getPreInboundNo(), receivedLines);
                    } catch (Exception ignored) {
                        log.info("Lines count updated failed " + ignored.getMessage());
                    }
