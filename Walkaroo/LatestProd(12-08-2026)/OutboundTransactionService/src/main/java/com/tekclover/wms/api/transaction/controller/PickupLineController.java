...
public class PickupLineController {
    ...
    public Stream<PickupLineV2> findPickupLineV2(@RequestBody SearchPickupLineV2 searchPickupLine)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchPickupLine.getPlantId().get(0),searchPickupLine.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
...
//    }
    ...
    public ResponseEntity<?> patchPickupLineBarcodeIdV2(@Valid @RequestBody UpdateBarcodeInput updateBarcodeInput) {
        try {
            String currentDB = baseService.getDataBase(updateBarcodeInput.getPlantId(),updateBarcodeInput.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
