...
@RestController
public class PickupHeaderController extends BaseService {
    ...
    public ResponseEntity<?> postPickupHeader(@Valid @RequestBody AddPickupHeader newPickupHeader, @RequestParam String loginUserID)
        ...
        try {
            String db = getDataBase(newPickupHeader.getPlantId(),newPickupHeader.getWarehouseId());
    ...
    public ResponseEntity<?> patchPickupHeader(@PathVariable String pickupNumber, @RequestParam String warehouseId,
            ...
            throws IllegalAccessException, InvocationTargetException {
        try {
            String db = getDataBase(updatePickupHeader.getPlantId(),updatePickupHeader.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(db);
            log.info("Current DB -------------> "  + db);
    ...
    public ResponseEntity<?> patchAssignedPickerIdInPickupHeader(@Valid @RequestBody List<UpdatePickupHeader> updatePickupHeaderList,
            ...
            throws IllegalAccessException, InvocationTargetException {
        try {
            String db = getDataBase(updatePickupHeaderList.get(0).getPlantId(),updatePickupHeaderList.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(db);
            log.info("Current DB -------------> "  + db);
    ...
    public Stream<PickupHeaderV2> findPickupHeaderV2(@RequestBody SearchPickupHeaderV2 searchPickupHeader)
            throws Exception {
        try {
            String db = getDataBase(searchPickupHeader.getPlantId().get(0),searchPickupHeader.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(db);
            log.info("Current DB -------------> "  + db);
