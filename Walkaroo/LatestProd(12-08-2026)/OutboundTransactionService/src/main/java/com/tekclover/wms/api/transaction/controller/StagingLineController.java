...
import com.tekclover.wms.api.transaction.model.dto.GroupedStagingLineResponse;
import com.tekclover.wms.api.transaction.model.dto.SAPData;
...
import com.tekclover.wms.api.transaction.service.ODataService;
...
public class StagingLineController {
    ...
    public ResponseEntity<?> postStagingLine(@Valid @RequestBody List<AddStagingLine> newStagingLine, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newStagingLine.get(0).getPlantId(),newStagingLine.get(0).getWarehouseId());
    ...
    public ResponseEntity<?> patchStagingLine(@PathVariable Long lineNo, @RequestParam String warehouseId,
            ...
            throws IllegalAccessException, InvocationTargetException {
        try {
            String currentDB = baseService.getDataBase(updateStagingLine.getPlantId(),updateStagingLine.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
...
//	}
    ...
    public Stream<StagingLineEntityV2> findStagingLineV2(@RequestBody SearchStagingLineV2 searchStagingLine) throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchStagingLine.getPlantId().get(0),searchStagingLine.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> stagingLineUpdate(@Valid @RequestBody List<StagingLineEntityV2> stagingLine, @RequestParam String loginUserID) throws Exception {
        try {
            String currentDB = baseService.getDataBase(stagingLine.get(0).getPlantId(),stagingLine.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
    ...
    public ResponseEntity<?> sapManualProcess(@Valid @RequestBody List<SAPData> sapData) throws Exception {
        try {
            String currentDB = baseService.getDataBase(sapData.get(0).getPlantId(),sapData.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
