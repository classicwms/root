...
public class OrderManagementLineController {
    ...
    public ResponseEntity<?> postOrderManagementLine(@Valid @RequestBody AddOrderManagementLine newOrderManagementLine, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newOrderManagementLine.getPlantId(),newOrderManagementLine.getWarehouseId());
    ...
    public ResponseEntity<?> patchOrderMangementLine(@PathVariable String refDocNumber,
            ...
            throws IllegalAccessException, InvocationTargetException {
        try {
            String currentDB = baseService.getDataBase(updateOrderMangementLine.getPlantId(),updateOrderMangementLine.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
...
//=======================================================V2============================================================================================
    ...
    public List<OrderManagementLineV2> findOrderManagementLineV2(@RequestBody SearchOrderManagementLineV2 searchOrderManagementLine)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchOrderManagementLine.getPlantId().get(0),searchOrderManagementLine.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> unallocateOrderManagementLineV2(@Valid @RequestBody List<OrderManagementLineV2> orderManagementLineV2, @RequestParam String loginUserID) throws Exception {
        try {
            String currentDB = baseService.getDataBase(orderManagementLineV2.get(0).getPlantId(),orderManagementLineV2.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> assignPickerV2(@RequestBody List<AssignPickerV2> assignPicker, @RequestParam String assignedPickerId,
        ...
        try {
            String currentDB = baseService.getDataBase(assignPicker.get(0).getPlantId(),assignPicker.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
