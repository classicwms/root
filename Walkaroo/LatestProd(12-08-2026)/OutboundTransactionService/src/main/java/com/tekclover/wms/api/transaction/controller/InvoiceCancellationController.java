...
public class InvoiceCancellationController {
    ...
    public ResponseEntity<?> cancelInboundOrder(@RequestBody InboundOrderCancelInput inboundOrderCancelInput,
        ...
        try {
            String currentDB = baseService.getDataBase(inboundOrderCancelInput.getPlantId(),inboundOrderCancelInput.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
	...
	public Stream<PickListHeader> findPickListHeader(@RequestBody SearchPickListHeader searchPickListHeader)
			throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchPickListHeader.getPlantId().get(0),searchPickListHeader.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
	...
	public Stream<SupplierInvoiceHeader> findSupplierInvoiceHeader(@RequestBody SearchSupplierInvoiceHeader searchSupplierInvoiceHeader)
			throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchSupplierInvoiceHeader.getPlantId().get(0),searchSupplierInvoiceHeader.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
