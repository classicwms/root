...
public class WarehouseSpecification implements Specification<Warehouse> {
	...
	@Override
     public Predicate toPredicate(Root<Warehouse> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
         ...
         if (searchWarehouse.getCompanyCodeId() != null && !searchWarehouse.getCompanyCodeId().isEmpty()) {
        	 predicates.add(cb.equal(root.get("companyCodeId"), searchWarehouse.getCompanyCodeId()));
         }
