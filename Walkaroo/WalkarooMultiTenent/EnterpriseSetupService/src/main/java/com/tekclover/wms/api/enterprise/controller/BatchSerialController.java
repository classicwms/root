...
public class BatchSerialController {
    ...
    public List<BatchSerial> findBatchSerial(@RequestBody SearchBatchSerial searchBatchSerial)
        ...
        try {
            DataBaseContextHolder.setCurrentDb("WK");
            String routingDb = dbConfigRepository.getDbNameList(searchBatchSerial.getCompanyId(), searchBatchSerial.getPlantId(), searchBatchSerial.getWarehouseId());
            log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(routingDb);
    ...
    public ResponseEntity<?> postBatchSerial(@Valid @RequestBody List<AddBatchSerial> newBatchSerial, @RequestParam String loginUserID)
        ...
        try {
            for (AddBatchSerial addBatchSerial : newBatchSerial) {
                DataBaseContextHolder.setCurrentDb("WK");
                String routingDb = dbConfigRepository.getDbName(addBatchSerial.getCompanyId(), addBatchSerial.getPlantId(), addBatchSerial.getWarehouseId());
                log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
                DataBaseContextHolder.clear();
                DataBaseContextHolder.setCurrentDb(routingDb);
            }
