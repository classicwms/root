...
//            "putaway_block = 0 and pick_block = 0 and status_id = :statusId and \n" +
    ...
    @Query(value = "SELECT top 1 PROPOSED_BIN FROM tblputawaystrategy WHERE ARTICLE = :article and c_id = :companyCode and plant_id = :plantId and \n" +
            "wh_id = :warehouseId and lang_id = :languageId and \n" +
            "(COALESCE(:gender, null) IS NULL OR (category IN (:gender))) and \n" +
    ...
    @Query(value = "SELECT TOP 1 super_max_capacity FROM tblputawaystrategy " +
            "WHERE c_id = :companyCode " +
            "AND plant_id = :plantId " +
            "AND lang_id = :languageId " +
            "AND wh_id = :warehouseId " +
            "AND article = :articleNo " +
            "AND is_deleted = 0 " +
            "AND gender = :gender " +
            "AND PROPOSED_BIN = :proposedBin " +
            "ORDER BY ctd_on DESC",  // <--- added for latest entry
            nativeQuery = true)
    String checkMaxAndSuperMaxCapacity(
            @Param("companyCode") String companyCode,
            @Param("plantId") String plantId,
            @Param("languageId") String languageId,
            @Param("warehouseId") String warehouseId,
            ...
            @Param("proposedBin") String proposedBin
    );


    @Query(value = "SELECT TOP 1 PROPOSED_BIN FROM tblputawaystrategy WHERE ARTICLE = :article " +
            "AND c_id = :companyCode AND plant_id = :plantId AND wh_id = :warehouseId AND lang_id = :languageId " +
            "AND is_deleted = 0 " +
            "AND gender = :gender " +
            "AND PROPOSED_BIN > :currentBin " +
            "ORDER BY PROPOSED_BIN ASC", nativeQuery = true)
    String getNextPutAwayStrategyStorageBin(
            @Param("companyCode") String companyCode,
            @Param("plantId") String plantId,
            @Param("languageId") String languageId,
            @Param("warehouseId") String warehouseId,
            @Param("gender") String gender,
            @Param("article") String article,
            @Param("currentBin") String currentBin
    );


    @Query(value = "SELECT TOP 1 PROPOSED_BIN FROM tblputawaystrategy " +
            "WHERE c_id = :companyCode " +
            "AND plant_id = :plantId " +
            "AND lang_id = :languageId " +
            "AND wh_id = :warehouseId " +
            ...
            "AND is_deleted = 0 " +
            "AND gender = :gender " +
            "ORDER BY ctd_on DESC", nativeQuery = true)
