...
import com.tekclover.wms.api.inbound.transaction.model.dto.GroupedStagingLineProjection;
...
//                                       String preInboundNo, String refDocNumber, String manufacturerCode);
    ...
    @Transactional
    @Modifying
    @Query(value = "update tblstagingline set VEHICLE_NO = :vehicleNo, STATUS_ID = :statusId, STATUS_TEXT = :text, ST_UTD_BY = :updatedBy, ST_UTD_ON = :updatedOn " +
            "where REF_DOC_NO = :refDocNo and IS_DELETED = 0 ", nativeQuery = true)
    int updateStagingLineVehicleNo(@Param("vehicleNo") String vehicleNo,
                                  @Param("statusId") Long statusId,
...
//                    "st.MANUFACTURER_FULL_NAME as manufacturerFullName, " +
                    ...
                    "            WHERE ph.REF_DOC_NO = st.REF_DOC_NO\n" +
                    "        )\n" +
                    "       THEN 'Created'\n" +
                    "        ELSE 'Not Created'\n" +
                    "    END as putAwayHeaderStatus "+
                    ...
                    " AND st.st_ctd_on BETWEEN :startCreatedOn and :endCreatedOn \n" +
//                    " AND (COALESCE(CONVERT(VARCHAR(225), :startCreatedOn), NULL) IS NULL OR " +
//                    " (st.st_ctd_on BETWEEN COALESCE(CONVERT(VARCHAR(225), :startCreatedOn), NULL) " +
//                    " AND COALESCE(CONVERT(VARCHAR(225), :endCreatedOn), NULL))) \n" +
