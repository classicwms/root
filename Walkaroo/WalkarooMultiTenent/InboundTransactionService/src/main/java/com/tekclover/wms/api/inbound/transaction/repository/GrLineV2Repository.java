...
public interface GrLineV2Repository extends JpaRepository<GrLineV2, Long>, JpaSpecificationExecutor<GrLineV2>,
    ...
    List<GrLineV2> findByRefDocNumberAndBarcodeIdAndDeletionIndicator(String refDocNo, String huSerialNo, long l);

    @Modifying
    @Query(value = "UPDATE tblgrline set status_id = :statusId, status_text = :statusText where c_id = :companyCodeId " +
    ...
    void updateGrLinePGI(@Param("companyCodeId") String companyCodeId,
                              @Param("plantId") String plantId,
                              @Param("warehouseId") String warehouseId,
                              @Param("refDocNumber") String refDocNumber,
    ...
    @Query(value = "SELECT * from tblgrline " +
            "WHERE barcode_id in (:barcodeId) and " +
            "(COALESCE(:itemCode, null) IS NULL OR (ITM_CODE IN (:itemCode)))", nativeQuery = true)
    List<GrLineV2> findGrLineValues(@Param("itemCode") String itemCode,
                                    @Param("barcodeId") String barcodeId);

    @Query(value = "SELECT * FROM tblgrline " +
        " WHERE barcode_id IN (:barcodeIds) " +
        "AND (:itemCode IS NULL OR ITM_CODE IN (:itemCode))", nativeQuery = true)
    List<GrLineV2> findGrLineBulk(
            @Param("itemCode") List<String> itemCode,
