...
@Slf4j
@Validated
@Api(tags = {"Configuration"}, value = "Configuration Operations related to ConfigurationController")
// label for swagger
...
public class ConfigurationController {
    ...
    public Stream<Configuration> findConfiguration(@RequestBody SearchConfiguration searchConfiguration) throws Exception {
        try {
            DataBaseContextHolder.setCurrentDb("WK");
            String routingDb = dbConfigRepository.getDbName(searchConfiguration.getCompanyCodeId(), searchConfiguration.getPlantId(), searchConfiguration.getWarehouseId());
            log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(routingDb);
    ...
    public ResponseEntity<?> postConfiguration(@Valid @RequestBody List<Configuration> newConfiguration, @RequestParam String loginUserID) throws Exception {
        try {
            for (Configuration configuration : newConfiguration) {
                DataBaseContextHolder.setCurrentDb("WK");
                String routingDb = dbConfigRepository.getDbName(configuration.getCompanyCodeId(), configuration.getPlantId(), configuration.getWarehouseId());
                log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
                DataBaseContextHolder.clear();
                DataBaseContextHolder.setCurrentDb(routingDb);
            }
