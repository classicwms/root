...
//            + "group by itm_code,mfr_name,plant_id,wh_id,c_id,lang_id) inventoryQuantity, \n"
    ...
    @Query(value = "select  PROP_ST_BIN proposedStorageBin, pal_id palletId, max(ref_doc_no) refDocNumber, max(pa_no) putAwayNumber, count(*) groupCount, max(ASS_USER_ID) assignedUserId, \n" +
            ...
            "and c_id in (:companyId) and plant_id in (:plantId) and wh_id in (:warehouseId) and is_deleted = 0 group by PROP_ST_BIN, pal_id ", nativeQuery = true)
    List<PutAwayPalletGroupResponse> findPutAwayHeaderValues(@Param("palletId") List<String> palletId,
                                                             @Param("statusId") Long statusId,
...
int getBarcodeId(
    ...
    @Query(value = "select * from tblputawayheader where WH_ID = :warehouseId AND pal_id = :palletId and PRE_IB_NO = :preInboundNo " +
            " and REF_DOC_NO = :refDocNumber and pa_no = :putAwayNumber and REF_FIELD_5 = :itemCode and PACK_BARCODE = :packBarcode and IS_DELETED = 0 ", nativeQuery = true)
    PutAwayHeaderV2 getPutAwayHeaderNew(@Param("warehouseId") String warehouseId,
