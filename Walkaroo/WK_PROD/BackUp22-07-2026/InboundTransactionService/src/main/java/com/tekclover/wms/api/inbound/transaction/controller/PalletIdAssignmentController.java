...
import com.tekclover.wms.api.inbound.transaction.service.PalletIdAssignmentService;
...
@Slf4j
@Validated
@Api(tags = {"PalletIdAssignment"}, value = "PalletIdAssignment  Operations related to PalletIdAssignmentController")
// label for swagger
...
public class PalletIdAssignmentController {
    ...
    public Stream<PalletIdAssignment> findPalletIdAssignmentNew(@Valid @RequestBody FindPalletIdAssignment findPalletIdAssignment) throws Exception {
        try {
            String currentDB = baseService.getDataBase(findPalletIdAssignment.getPlantId().get(0),findPalletIdAssignment.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
