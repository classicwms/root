...
//            log.info("warehouse -> {}",iKeyValuePair.getWarehouse());
            ...
            inboundOrderProcess.setCompanyCode("1000");
            inboundOrderProcess.setToCompanyCode("1000");
            inboundOrderProcess.setBranchCode(asn.getPlant());
            inboundOrderProcess.setToBranchCode(asn.getPlant());
            inboundOrderProcess.setLanguageId("EN");
//            inboundOrderProcess.setWarehouseId(iKeyValuePair.getWarehouse());
//            inboundOrderProcess.setWarehouseId(asn.getWarehouseId());

            if (asn.getStorageLocation() != null && !asn.getStorageLocation().isEmpty()) {
                inboundOrderProcess.setWarehouseId(asn.getStorageLocation());
            } else {
                throw new BadRequestException("Warehouse ID is missing for ASN: " + asn.getInbound());
            }
...
//			log.info("warehouse -> {}", iKeyValuePair.getWarehouse());
            ...
            obOrderProcessV4.setLanguageId("EN");
            obOrderProcessV4.setCompanyCode("1000");
            obOrderProcessV4.setBranchCode(so.getBranchCode());
////			obOrderProcessV4.setWarehouseId(iKeyValuePair.getWarehouse());
//            obOrderProcessV4.setWarehouseId(so.getWarehouseId());

            if (so.getWarehouseId() != null && !so.getWarehouseId().isEmpty()) {
                obOrderProcessV4.setWarehouseId(so.getWarehouseId());
            } else {
                throw new RuntimeException("Warehouse Id is mandatory");
            }
...
//			log.info("warehouse -> {}", iKeyValuePair.getWarehouse());
            ...
            if (sapOrder.getWarehouseId() != null && !sapOrder.getWarehouseId().isEmpty()) {
                obOrderProcessV4.setWarehouseId(sapOrder.getWarehouseId());
            } else {
                throw new RuntimeException("Warehouse Id is mandatory");
            }
