...
public class PeriodicLineController {
    ...
    public List<PeriodicLine> findPeriodicLine(@RequestBody SearchPeriodicLine searchPeriodicLine)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchPeriodicLine.getPlantId(),searchPeriodicLine.getWarehouseId());
    ...
    public ResponseEntity<?> patchPeriodicLine(@PathVariable String cycleCountNo,
        ...
        try {
            String currentDB = baseService.getDataBase(updatePeriodicLine.get(0).getPlantId(),updatePeriodicLine.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public List<PeriodicLineV2> findPeriodicLineV2(@RequestBody SearchPeriodicLineV2 searchPeriodicLineV2)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchPeriodicLineV2.getPlantId().get(0),searchPeriodicLineV2.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
