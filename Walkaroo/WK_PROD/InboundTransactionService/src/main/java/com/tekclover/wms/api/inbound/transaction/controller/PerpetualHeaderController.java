...
import com.tekclover.wms.api.inbound.transaction.model.cyclecount.perpetual.v2.PerpetualHeaderV2;
import com.tekclover.wms.api.inbound.transaction.model.cyclecount.perpetual.v2.SearchPerpetualHeaderV2;
...
import com.tekclover.wms.api.inbound.transaction.service.PerpetualHeaderService;
...
public class PerpetualHeaderController {
    ...
    public ResponseEntity<?> postPerpetualHeader(@Valid @RequestBody AddPerpetualHeader newPerpetualHeader,
        ...
        try {
            String currentDB = baseService.getDataBase(newPerpetualHeader.getPlantId(),newPerpetualHeader.getWarehouseId());
    ...
    public ResponseEntity<?> patchPerpetualHeader(@PathVariable String cycleCountNo, @RequestParam String warehouseId,
        ...
        try {
            String currentDB = baseService.getDataBase(updatePerpetualHeader.getPlantId(),updatePerpetualHeader.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    @ApiOperation(response = PerpetualHeaderEntityV2.class, value = "Get all PerpetualHeader details")
    // label for swagger
    @GetMapping("/v2")
    ...
    public Stream<PerpetualHeaderV2> findPerpetualHeaderV2(@RequestBody SearchPerpetualHeaderV2 searchPerpetualHeader)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchPerpetualHeader.getPlantId().get(0),searchPerpetualHeader.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
