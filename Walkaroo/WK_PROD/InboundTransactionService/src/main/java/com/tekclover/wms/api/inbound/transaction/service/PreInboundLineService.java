...
import com.tekclover.wms.api.inbound.transaction.model.inbound.preinbound.v2.*;
...
import com.tekclover.wms.api.inbound.transaction.repository.specification.PreInboundLineV2Specification;
...
//		log.info("results: " + results);
     ...
     * @throws ParseException
     */
    public List<PreInboundLineOutputReport> findPreInboundLineReport(SearchPreInboundLineV2 searchPreInboundLine) throws ParseException {
        if (searchPreInboundLine.getStartCreatedOn() != null && searchPreInboundLine.getEndCreatedOn() != null) {
            Date[] dates = DateUtils.addTimeToDatesForSearch(searchPreInboundLine.getStartCreatedOn(), searchPreInboundLine.getEndCreatedOn());
            searchPreInboundLine.setStartCreatedOn(dates[0]);
            searchPreInboundLine.setEndCreatedOn(dates[1]);
        ...
        PreInboundLineV2Specification spec = new PreInboundLineV2Specification(searchPreInboundLine);
        List<PreInboundLineEntityV2> results = preInboundLineV2Repository.stream(spec, PreInboundLineEntityV2.class).collect(Collectors.toList());
        List<PreInboundLineOutputReport> preInboundLineOutputList = new ArrayList<>();
        if(results != null && !results.isEmpty()) {
            for(PreInboundLineEntityV2 preInboundLine : results) {
                PreInboundLineOutputReport newPreInboundLineOutput = new PreInboundLineOutputReport();
                BeanUtils.copyProperties(preInboundLine, newPreInboundLineOutput, CommonUtils.getNullPropertyNames(preInboundLine));

                // Getting Max and SuperMax Capacities
                PutAwayStrategyDetails putAwayStrategyDetails = preInboundLineV2Repository.getPutAwayStrategyDetails(preInboundLine.getArticleNo(), preInboundLine.getGender());
                newPreInboundLineOutput.setMaxCapacity(putAwayStrategyDetails.getMaxCapacity());
                newPreInboundLineOutput.setSuperMaxCapacity(putAwayStrategyDetails.getSuperMaxCapacity());
                newPreInboundLineOutput.setStorageBin(putAwayStrategyDetails.getStorageBin());

                List<InventoryDetailsV2> inventoryDetails = new ArrayList<>();
                SearchInventoryV2 searchInventoryV2 = getSearchInventoryV2(preInboundLine);
                List<IInventoryImpl> inventoryList = inventoryService.findInventoryNewV2(searchInventoryV2);
                if(inventoryList != null && !inventoryList.isEmpty()) {
                    for (IInventoryImpl inventory : inventoryList) {
                        InventoryDetailsV2 inventoryDetail = new InventoryDetailsV2();
                        inventoryDetail.setStorageBin(inventory.getStorageBin());
                        inventoryDetail.setInventoryQty(inventory.getReferenceField4());
                        inventoryDetail.setAccommodationQty(Double.valueOf(newPreInboundLineOutput.getSuperMaxCapacity()) - inventoryDetail.getInventoryQty());
                        inventoryDetails.add(inventoryDetail);
                    }
                }
