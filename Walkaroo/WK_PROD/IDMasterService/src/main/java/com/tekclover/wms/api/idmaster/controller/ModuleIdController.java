...
public class ModuleIdController {
    ...
    public ResponseEntity<?> postModuleId(@Valid @RequestBody List<AddModuleId> newModuleId,
        ...
        try {
            for (AddModuleId addModuleId : newModuleId) {
                DataBaseContextHolder.setCurrentDb("WK");
                String routingDb = dbConfigRepository.getDbName(addModuleId.getCompanyCodeId(), addModuleId.getPlantId(), addModuleId.getWarehouseId());
                log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
                DataBaseContextHolder.clear();
                DataBaseContextHolder.setCurrentDb(routingDb);
            }
