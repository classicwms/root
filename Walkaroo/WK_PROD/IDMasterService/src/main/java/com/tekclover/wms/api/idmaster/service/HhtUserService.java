...
//			}
    ...
    public HhtUser getHhtUserV2(String userId, String userName, String warehouseId, String companyCodeId, String plantId, String languageId) {
        Optional<HhtUser> hhtUser = hhtUserRepository.findByUserIdAndUserNameAndWarehouseIdAndCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                userId,
                userName,
                warehouseId,
                companyCodeId,
        ...
        BeanUtils.copyProperties(hhtUser.get(), dbHhtUser, CommonUtils.getNullPropertyNames(hhtUser.get()));
        if (hhtUser.get().getOrderTypeIds() != null) {
            List<String> orderTypeId = new ArrayList<>();
            for (OrderTypeId dbOrderTypeId : hhtUser.get().getOrderTypeIds()) {
                orderTypeId.add(dbOrderTypeId.getOrderTypeId());
...
//    }
    ...
    public HhtUser createHhtUser(AddHhtUser newHhtUser, String loginUserID)
        ...
        Optional<HhtUser> duplicateHhtUser = hhtUserRepository.findByUserIdAndUserNameAndWarehouseIdAndCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
                newHhtUser.getUserId(),
                newHhtUser.getUserName(),
        ...
        if (!duplicateHhtUser.isEmpty()) {
            throw new IllegalAccessException("UserName is getting Duplicated");
        }
        IKeyValuePair iKeyValuePair =
    ...
    public List<HhtUser> updateHhtUserV2(List<UpdateHhtUser> updateHhtUserList, String loginUserID)
            throws IllegalAccessException, InvocationTargetException, ParseException {
        List<HhtUser> HhtUserLists = new ArrayList<>();
        for (UpdateHhtUser updateHhtUser : updateHhtUserList) {
            hhtUserRepository.deleteHhtUserList(updateHhtUser.getUserId(),updateHhtUser.getLanguageId(), updateHhtUser.getCompanyCodeId(), updateHhtUser.getPlantId() , updateHhtUser.getWarehouseId());
            log.info("HhtUser Delete SuccessFully " + updateHhtUser.getUserId());
        }
        for (UpdateHhtUser updateHhtUser : updateHhtUserList) {
            log.info("updateHhtUser ------> " + updateHhtUser);
            HhtUser newHhtUser = new HhtUser();
            BeanUtils.copyProperties(updateHhtUser, newHhtUser, CommonUtils.getNullPropertyNames(updateHhtUser));
            newHhtUser.setLanguageId(updateHhtUser.getLanguageId());
            newHhtUser.setCompanyCodeId(updateHhtUser.getCompanyCodeId());
            newHhtUser.setWarehouseId(updateHhtUser.getWarehouseId());
            newHhtUser.setLevelId(updateHhtUser.getLevelId());
            newHhtUser.setUpdatedBy(loginUserID);
            newHhtUser.setDeletionIndicator(0L);
            newHhtUser.setUpdatedOn(new Date());
            hhtUserRepository.save(newHhtUser);
            HhtUserLists.add(newHhtUser);
        }
        return HhtUserLists;
    }
     ...
     * @param loginUserID
     */
    public void deleteHhtUserV2(List<HhtUser> deleteHhtUser, String loginUserID) {
        if(deleteHhtUser != null && !deleteHhtUser.isEmpty()){
            log.info("given values to delete HhtUser---> {}", deleteHhtUser);
            deleteHhtUser.parallelStream().forEach(deleteInput -> {
                HhtUser dbHhtUser = getHhtUserV2(deleteInput.getUserId(),deleteInput.getUserName(), deleteInput.getWarehouseId(),
                        deleteInput.getCompanyCodeId(), deleteInput.getPlantId(), deleteInput.getLanguageId());
                dbHhtUser.setDeletionIndicator(1L);
                dbHhtUser.setUpdatedBy(loginUserID);
                hhtUserRepository.save(dbHhtUser);
            });
        } else {
            throw new EntityNotFoundException("Error in deleting Id: " + deleteHhtUser);
        }
    }
