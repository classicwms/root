...
public class InboundHeaderController {
    ...
    public ResponseEntity<?> postInboundHeader(@Valid @RequestBody AddInboundHeader newInboundHeader,
        ...
        try {
            String currentDB = baseService.getDataBase(newInboundHeader.getPlantId(),newInboundHeader.getWarehouseId());
    ...
    public ResponseEntity<?> patchInboundHeader(@PathVariable String refDocNumber, @RequestParam String warehouseId,
        ...
        try {
            String currentDB = baseService.getDataBase(updateInboundHeader.getPlantId(),updateInboundHeader.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public List<InboundHeaderV2> findInboundHeaderV2(@RequestBody SearchInboundHeaderV2 searchInboundHeader)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchInboundHeader.getPlantId().get(0),searchInboundHeader.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
