...
//            @Param(value = "stockTypeText") String stockTypeText);
    ...
    List<InventoryV2> findAllByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerNameAndBinClassIdAndDeletionIndicator(
...
//            + "COALESCE(INV_QTY,0)+COALESCE(ALLOC_QTY,0) referenceField4, \n"
    ...
    @Query(value = "select max(inv_id) inventoryId into #inv from tblinventory \n" +
          ...
          //  "(COALESCE(:binClassId, null) IS NULL OR (BIN_CL_ID IN (:binClassId))) and\n" +
            ...
            "(COALESCE(:storageBin, null) IS NULL OR (iv.ST_BIN IN (:storageBin))) and\n" +
            "(COALESCE(:binClassId, null) IS NULL OR (iv.BIN_CL_ID IN (:binClassId))) and\n" +
            "iv.is_deleted = 0 and iv.INV_QTY > 0 and iv.stck_typ_id = :stockTypeId \n", nativeQuery = true)
    ...
    @Query(value = "select max(inv_id) inventoryId into #inv from tblinventory \n" +
            ...
            + "SELECT SUM(INV_QTY) AS inventoryQty FROM tblinventory \r\n"
            + "WHERE WH_ID = :warehouseId and ITM_CODE = :itemCode AND BIN_CL_ID in (:binClassId) AND STCK_TYP_ID = :stockTypeId \r\n"
            + "AND C_ID = :companyCodeId and PLANT_ID = :plantId AND LANG_ID = :languageId \r\n"
            + "AND MFR_NAME = :manufacturerName AND IS_DELETED = 0 \r\n"
            + "AND INV_ID in (select inventoryId from #inv) \r\n"
            + "GROUP BY ITM_CODE, MFR_NAME, PLANT_ID, WH_ID, C_ID, LANG_ID \r\n"
            + "HAVING SUM(INV_QTY) > 0 \r\n"
            + "ORDER BY SUM(INV_QTY)", nativeQuery = true)
    public List<IInventory> findInventoryListFFMO(@Param("companyCodeId") String companyCodeId,
                                                          @Param("plantId") String plantId,
                                                          @Param("languageId") String languageId,
                                                          @Param("warehouseId") String warehouseId,
                                                          @Param("itemCode") String itemCode,
    ...
    @Query(value = "SELECT SUM(INV_QTY) AS inventoryQty FROM tblinventory \r\n"
            + "WHERE WH_ID = :warehouseId and ITM_CODE = :itemCode AND BIN_CL_ID in (:binClassId) AND STCK_TYP_ID = :stockTypeId \r\n"
            + "AND C_ID = :companyCodeId and PLANT_ID = :plantId AND LANG_ID = :languageId \r\n"
            + "AND MFR_NAME = :manufacturerName AND IS_DELETED = 0 \r\n"
            + "AND INV_ID in (select max(inv_id) inventoryId from tblinventory WHERE \n" +
            "(COALESCE(:itemCode, null) IS NULL OR (ITM_CODE IN (:itemCode))) and \n" +
            "(COALESCE(:binClassId, null) IS NULL OR (BIN_CL_ID IN (:binClassId))) and \n" +
            "(COALESCE(:manufacturerName, null) IS NULL OR (MFR_NAME IN (:manufacturerName))) and \n" +
            "(COALESCE(:companyCodeId, null) IS NULL OR (c_id IN (:companyCodeId))) and \n" +
            ...
            + "HAVING SUM(INV_QTY) > 0 \r\n"
            + "ORDER BY SUM(INV_QTY)", nativeQuery = true)
    public List<Long> findInventoryQty(@Param("companyCodeId") String companyCodeId,
                                                  @Param("plantId") String plantId,
                                                  @Param("languageId") String languageId,
                                                  @Param("warehouseId") String warehouseId,
                                                  @Param("itemCode") String itemCode,
    ...
    //   "iv.inv_id in" +
    //            "(select max(inv_id) inventoryId from tblinventory \n" +
    //            "WHERE \n" +
    //            "(COALESCE(:itemCode, null) IS NULL OR (ITM_CODE IN (:itemCode))) and \n" +
    //            "(COALESCE(:binClassId, null) IS NULL OR (BIN_CL_ID IN (:binClassId))) and\n" +
    //            "(COALESCE(:manufacturerName, null) IS NULL OR (MFR_NAME IN (:manufacturerName))) and \n" +
...
//            "group by itm_code,barcode_id,mfr_name,pack_barcode,stck_typ_id,st_bin,plant_id,wh_id,c_id,lang_id \n" +
    ...
    @Query(value = "SELECT * FROM tblinventory where c_id =:companyCodeId and plant_id =:plantId and lang_id =:languageId AND \n" +
            "wh_id =:warehouseId and itm_code =:itemCode and mfr_name =:manufacturerName and barcode_id=:barcodeId and ref_doc_no =:referenceDocumentNo and IS_DELETED=0",nativeQuery = true)
    InventoryV2 checkForBarcodeId(@Param("companyCodeId") String companyCodeId,
                                  @Param("plantId") String plantId,
                                  @Param("languageId") String languageId,
                                  @Param("warehouseId") String warehouseId,
                                  @Param("itemCode") String itemCode,
    ...
    @Modifying
    @Query(value = "UPDATE top 1 from tblinventory SET inv_qty = 0, ref_field_4 = 0 where c_id = :companyCodeId " +
            "AND plant_id = :plantId AND wh_id = :warehouseId " +
            "AND itm_code = :itemCode order by inv_id desc", nativeQuery = true)
    public void updateInventoryForPGI(@Param("companyCodeId") String companyCodeId,
                                      @Param("plantId") String plantId,
                                      @Param("warehouseId") String warehouseId,
                                      @Param("itemCode") String itmCode);

    @Query(value = "SELECT COUNT(*) FROM tblinventory WHERE st_bin = :storageBin", nativeQuery = true)
    Long countByStorageBin(@Param("storageBin") String storageBin);
  ...
  //------Added-by--Murugavel--on--10/03---for---InventoryTrans-----------
   	...
   	@Modifying
    @Query(value = "UPDATE tblinventory SET inv_qty = 0, ref_field_4 = 0, alloc_qty = 0 " +
            "where barcode_id = :barcode_id and bin_cl_id = 8", nativeQuery = true)
    public void updateInventoryV3(@Param("barcode_id") String barcode_id);

	List<InventoryV2> findByBarcodeIdAndBinClassIdAndDeletionIndicatorOrderByInventoryIdDesc(String barcodeId,
			Long binClassId, long l);
    ...
    @Query(value = "SELECT \n" +
            ...
            //   "(COALESCE(:binClassId, null) IS NULL OR (iv.BIN_CL_ID IN (:binClassId))) and\n" +
            "(COALESCE(:stockTypeId, null) IS NULL OR (iv.STCK_TYP_ID IN (:stockTypeId))) and \n" +
            "iv.is_deleted = 0 and (iv.INV_QTY > 0) order by iv.REF_FIELD_10 \n", nativeQuery = true)
    public List<IInventoryImpl> getOMLInventoryFullFillmentMtoNumber(@Param("companyCodeId") String companyCodeId,
                                                                     @Param("plantId") String plantId,
                                                                     @Param("languageId") String languageId,
                                                                     @Param("warehouseId") String warehouseId,
                                                                     @Param("barcodeId") String barcodeId,
    ...
    @Query(value = "SELECT * from tblinventory \n" +
            ...
            "(COALESCE(:binClassId, null) IS NULL OR (BIN_CL_ID IN (:binClassId))) and\n" +
            "(COALESCE(:mtoNumber, null) IS NULL OR (MTO_NUMBER IN (:mtoNumber))) and\n" +
            "(COALESCE(:itemCode, null) IS NULL OR (ITM_CODE IN (:itemCode))) and \n" +
            "(COALESCE(:stockTypeId, null) IS NULL OR (STCK_TYP_ID IN (:stockTypeId))) and \n" +
            "is_deleted = 0 and (INV_QTY > 0) order by BIN_CL_ID desc \n", nativeQuery = true)
    public List<InventoryV2> getOMLInventoryFFMMtoNumber(@Param("companyCodeId") String companyCodeId,
                                                         @Param("plantId") String plantId,
                                                         @Param("languageId") String languageId,
                                                         @Param("warehouseId") String warehouseId,
                                                         @Param("barcodeId") String barcodeId,
                                                         ...
                                                         @Param("binClassId") List<Long> binClassId,
                                                         @Param("stockTypeId") Long stockTypeId,
                                                         @Param("mtoNumber") String mtoNumber);
...
//                                           @Param("storageBin") List<String> storageBin);
    ...
    @Query(value = "select * from tblinventory where ref_field_4 > 0 and " +
            "inv_id in (select max(inv_id) from tblinventory where is_deleted = 0 group by barcode_id, itm_code) and " +
            "barcode_id = :barcodeId and c_id = :companyCodeId and lang_id = :languageId and " +
            "plant_id = :plantId and wh_id = :warehouseId  ", nativeQuery = true)
    InventoryV2 getInventoryForTransferNew(@Param("companyCodeId") String companyCodeId,
                                           @Param("plantId") String plantId,
                                           @Param("languageId") String languageId,
                                           @Param("warehouseId") String warehouseId,
                                           @Param("barcodeId") String barcodeId);
