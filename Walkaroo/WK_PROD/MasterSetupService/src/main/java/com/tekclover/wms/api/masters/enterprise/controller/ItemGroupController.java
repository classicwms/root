...
import com.tekclover.wms.api.masters.model.itemgroup.AddItemGroup;
import com.tekclover.wms.api.masters.model.itemgroup.ItemGroup;
import com.tekclover.wms.api.masters.model.itemgroup.SearchItemGroup;
import com.tekclover.wms.api.masters.enterprise.service.ItemGroupService;
...
public class ItemGroupController {
    ...
    public List<ItemGroup> findItemGroup(@RequestBody SearchItemGroup searchItemGroup)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchItemGroup.getPlantId(), searchItemGroup.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postItemGroup(@Valid @RequestBody List<AddItemGroup> newItemGroup, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newItemGroup.get(0).getPlantId(), newItemGroup.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
