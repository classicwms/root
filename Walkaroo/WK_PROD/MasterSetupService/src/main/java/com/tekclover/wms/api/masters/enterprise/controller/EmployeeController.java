...
import com.tekclover.wms.api.masters.model.employee.AddEmployee;
import com.tekclover.wms.api.masters.model.employee.Employee;
import com.tekclover.wms.api.masters.model.employee.UpdateEmployee;
import com.tekclover.wms.api.masters.enterprise.service.EmployeeService;
...
public class EmployeeController {
    ...
    public ResponseEntity<?> postEmployee(@Valid @RequestBody AddEmployee newEmployee, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newEmployee.getPlantId(), newEmployee.getWarehouseId());
    ...
    public ResponseEntity<?> patchEmployee(@PathVariable String employeeId, @RequestParam String warehouseId,
            ...
            throws IllegalAccessException, InvocationTargetException, ParseException {
        try {
            String currentDB = baseService.getDataBase(updateEmployee.getPlantId(), updateEmployee.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
