...
@RestController
public class HandlingEquipmentController extends BaseService {
    ...
    public List<HandlingEquipment> findHandlingEquipment(@RequestBody SearchHandlingEquipment searchHandlingEquipment)
            throws Exception {
        try {
            String db = getDataBase(searchHandlingEquipment.getPlantId().get(0), searchHandlingEquipment.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(db);
            log.info("Current DataBase --------------> " + db);
    ...
    public ResponseEntity<?> postHandlingEquipment(@Valid @RequestBody AddHandlingEquipment newHandlingEquipment,
        ...
        try {
            String db = getDataBase(newHandlingEquipment.getPlantId(), newHandlingEquipment.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(db);
            log.info("Current DataBase --------------> " + db);
