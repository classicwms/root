...
public class StorageBinController {
    ...
    public List<StorageBinListImpl> getStorageBinLikeSearchV2(@Valid @RequestBody LikeSearchInput likeSearchInput)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(likeSearchInput.getPlantId(), likeSearchInput.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> getStorageBin(@RequestBody StorageBinPutAway storageBinPutAway) {
        try {
            String currentDB = baseService.getDataBase(storageBinPutAway.getPlantId(), storageBinPutAway.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public List<StorageBin> findStorageBin(@RequestBody SearchStorageBin searchStorageBin)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchStorageBin.getPlantId().get(0), searchStorageBin.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postStorageBin(@Valid @RequestBody AddStorageBin newStorageBin, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newStorageBin.getPlantId(), newStorageBin.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
