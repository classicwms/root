...
public class StrategiesController {
    ...
    public List<Strategies> findStrategies(@RequestBody SearchStrategies searchStrategies)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchStrategies.getPlantId(), searchStrategies.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postStrategies(@Valid @RequestBody List<AddStrategies> newStrategies, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newStrategies.get(0).getPlantId(), newStrategies.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
