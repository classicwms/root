...
public class BatchSerialController {
    ...
    public List<BatchSerial> findBatchSerial(@RequestBody SearchBatchSerial searchBatchSerial)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchBatchSerial.getPlantId().get(0), searchBatchSerial.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postBatchSerial(@Valid @RequestBody List<AddBatchSerial> newBatchSerial, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newBatchSerial.get(0).getPlantId(), newBatchSerial.get(0).getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
