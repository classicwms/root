...
public class BusinessPartnerController {
    ...
    public List<BusinessPartner> findBusinessPartner(@RequestBody SearchBusinessPartner searchBusinessPartner)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchBusinessPartner.getPlantId().get(0),
                    searchBusinessPartner.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postBusinessPartner(@Valid @RequestBody AddBusinessPartner newBusinessPartner, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newBusinessPartner.getPlantId(), newBusinessPartner.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
