...
public class FloorController {
    ...
    public List<Floor> findFloor(@RequestBody SearchFloor searchFloor)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchFloor.getPlantId(), searchFloor.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postFloor(@Valid @RequestBody AddFloor newFloor, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newFloor.getPlantId(), newFloor.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
