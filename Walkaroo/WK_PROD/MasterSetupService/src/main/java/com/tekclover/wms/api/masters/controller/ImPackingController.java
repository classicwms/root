...
//    	log.info("ImPacking : " + impacking);
    ...
    public List<ImPacking> findImPacking(@RequestBody SearchImPacking searchImPacking)
            throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchImPacking.getPlantId().get(0), searchImPacking.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> postImPacking(@Valid @RequestBody AddImPacking newImPacking, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newImPacking.getPlantId(), newImPacking.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
