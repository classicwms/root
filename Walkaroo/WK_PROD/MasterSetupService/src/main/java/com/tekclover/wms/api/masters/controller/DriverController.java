...
public class DriverController {
    ...
    public ResponseEntity<?> postDriver(@Valid @RequestBody AddDriver newDriver, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newDriver.getPlantId(), newDriver.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> findDriver(@Valid @RequestBody SearchDriver searchDriver) throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchDriver.getPlantId().get(0), searchDriver.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
