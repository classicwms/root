...
public class DockController {
    ...
    public ResponseEntity<?> postDock(@Valid @RequestBody AddDock newDock, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newDock.getPlantId(), newDock.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> findDock(@Valid @RequestBody SearchDock searchDock) throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchDock.getPlantId().get(0), searchDock.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
