...
public class RouteController {
    ...
    public ResponseEntity<?> postRoute(@Valid @RequestBody AddRoute newRoute, @RequestParam String loginUserID)
        ...
        try {
            String currentDB = baseService.getDataBase(newRoute.getPlantId(), newRoute.getWarehouseId());
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
    ...
    public ResponseEntity<?> findRoute(@Valid @RequestBody SearchRoute searchRoute) throws Exception {
        try {
            String currentDB = baseService.getDataBase(searchRoute.getPlantId().get(0), searchRoute.getWarehouseId().get(0));
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(currentDB);
            log.info("Current DB " + currentDB);
