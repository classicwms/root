...
//			log.info("result : " + result.getStatusCode());
	...
	public HhtUser[] updateHhtUserList(List<UpdateHhtUser> modifiedHhtUser, String loginUserID,
								 String authToken) {
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
			...
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getIDMasterServiceApiUrl() + "hhtuser/update/list/v2")
					.queryParam("loginUserID", loginUserID);

			ResponseEntity<HhtUser[]> result = restTemplate.exchange(builder.toUriString(), HttpMethod.POST, entity, HhtUser[].class);
			log.info("result : " + result.getStatusCode());
			return result.getBody();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean deleteHhtUserV2(List<HhtUser> hhtUserList, String loginUserID, String authToken) {
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
			headers.add("User-Agent", "MNRClara's RestTemplate");
			...
			HttpEntity<?> entity = new HttpEntity<>(hhtUserList, headers);
			UriComponentsBuilder builder =
					UriComponentsBuilder.fromHttpUrl(getIDMasterServiceApiUrl() + "hhtuser/delete/list/v2")
							.queryParam("loginUserID", loginUserID);
