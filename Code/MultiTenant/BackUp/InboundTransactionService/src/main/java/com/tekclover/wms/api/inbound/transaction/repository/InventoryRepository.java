...
public interface InventoryRepository extends PagingAndSortingRepository<Inventory, Long>,
    ...
    // referenceField10
    List<InventoryV2> findByWarehouseIdAndItemCodeAndBinClassIdAndReferenceField10InAndDeletionIndicator(String warehouseId,
...
//	Stream<InventoryImpl> findInventory (
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE tblinventory \n" +
            "SET REMAINING_DAYS = CASE WHEN DATEDIFF(DAY, GETDATE(), EXP_DATE) - 1 < 0 THEN 0 ELSE DATEDIFF(DAY, GETDATE(), EXP_DATE) - 1 END, \n" +
            "REMAINING_SELF_LIFE_PERCENTAGE = CASE WHEN SELF_LIFE IS NULL OR TRY_CAST(SELF_LIFE AS FLOAT) = 0 THEN 0 ELSE CASE WHEN DATEDIFF(DAY, GETDATE(), EXP_DATE) - 1 < 0 THEN 0 ELSE ROUND(((DATEDIFF(DAY, GETDATE(), EXP_DATE) - 1) * 100.0) / TRY_CAST(SELF_LIFE AS FLOAT), 2) END END \n" +
            "WHERE IS_DELETED = 0 AND BIN_CL_ID IN (7, 1)", nativeQuery = true)
    int updateShelfLife();
