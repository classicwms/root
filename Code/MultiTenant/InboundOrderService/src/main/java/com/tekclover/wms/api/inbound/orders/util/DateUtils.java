package com.tekclover.wms.api.inbound.orders.util;
...
//			return Date.from(fromDeliveryDate_d.atStartOfDay(ZoneId.systemDefault()).toInstant());
    ...
    public static Date parseCustomDate(String input) {
        try {
            ...
            // Parse components
            String day = input.substring(0, 2);             // "03"
            char monthLetter = input.charAt(2);             // 'D'
            String year = "20" + input.substring(3);        // "2025"

            // Convert month letter to month number
            int month = monthLetter - 'A' + 1;              // 'D' - 'A' + 1 = 4
            String monthStr = String.format("%02d", month); // "04"

            // Get current time
            LocalTime now = LocalTime.now();
            String timeStr = now.format(DateTimeFormatter.ofPattern("HH:mm:ss")); // "14:32:10"

            // Full datetime string
            String formattedDateTime = day + "/" + monthStr + "/" + year + " " + timeStr;
            ...
            log.error("Error parsing barcode date from input: " + input, e);
            return null;
        }
    }
