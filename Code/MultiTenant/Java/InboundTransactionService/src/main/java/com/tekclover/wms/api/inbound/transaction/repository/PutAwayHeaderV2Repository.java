...
public interface PutAwayHeaderV2Repository extends JpaRepository<PutAwayHeaderV2, Long>,
    ...
    PutAwayHeaderV2 findByPutAwayNumberAndBarcodeIdAndPalletCodeAndDeletionIndicator(String putAwayNumber, String barcodeId, String palletCode, Long deletionIndicator);
    ...
    @Query(
            value = "SELECT TOP 1 * FROM tblputawayheader " +
                    "WHERE c_id = :companyCodeId " +
                    "AND plant_id = :plantId " +
                    "AND wh_id = :warehouseId " +
                    "AND lang_id = :languageId " +
                    "AND ref_field_5 = :itemCode " +
                    "AND mfr_name = :manufacturerName " +
                    "AND status_id = :statusId " +
                    "AND barcode_id = :barcodeId " +
                    "AND is_deleted = 0 " +
                    "ORDER BY PA_CTD_ON DESC",
            nativeQuery = true
    )
    PutAwayHeaderV2 findLatestPutawayHeader(
    ...
    @Query(value = "SELECT * from tblputawayheader WHERE c_id = :companyCode \n" +
            "AND plant_id = :plantId AND wh_id = :warehouseId AND lang_id = :languageId \n" +
            "AND ref_field_5 = :itemCode AND mfr_name = :manufacturerName AND status_id = :statusId \n" +
            "AND is_deleted = 0", nativeQuery = true)
    PutAwayHeaderV2 getExistingPutAwayHeaderForKnowell(@Param("companyCode") String companyCode,
                                                       @Param("plantId") String plantId,
                                                       @Param("warehouseId") String warehouseId,
                                                       @Param("languageId") String languageId,
                                                       @Param("itemCode") String itemCode,
    ...
    @Query(value = "select PROP_ST_BIN from tblputawayheader where c_id=:companyCodeId and plant_id=:plantId and lang_Id=:languageId and wh_id=:warehouseId and \n" +
            " status_id = :statusId and is_deleted = 0 group by PROP_ST_BIN", nativeQuery = true)
    public List<String> getPutAwayHeaderProposedBinsV7(@Param("companyCodeId") String companyCodeId,
                                                     @Param("plantId") String plantId,
                                                     @Param("languageId") String languageId,
                                                     @Param("warehouseId") String warehouseId,
                                                     @Param("statusId") Long statusId);
    ...
    @Modifying
    @Query(value = "UPDATE tblgrline SET PA_NO = :putAwayNo WHERE \n" +
            "c_id = :companyCodeId and \n" +
            "lang_id = :languageId and \n" +
            "plant_id = :plantId and \n" +
            "wh_id = :warehouseId and \n" +
            "ref_doc_no = :refDocNumber AND itm_code = :itemCode AND ib_line_no = :lineNumber AND \n" +
            "gr_ctd_on = :grCtdOn AND pre_ib_no = :preInboundNo AND IS_DELETED = 0", nativeQuery = true)
    void updatePutAwayNumber(@Param("companyCodeId") String companyCode,
                             @Param("plantId") String plantId,
                             @Param("languageId") String languageId,
                             @Param("warehouseId") String warehouseId,
                             @Param("refDocNumber") String refDocNumber,
    ...
    PutAwayHeaderV2 findTopByCompanyCodeIdAndPlantIdAndWarehouseIdAndReferenceField5AndManufacturerNameAndReferenceField9AndLanguageIdAndPutAwayNumberAndDeletionIndicator(
            String companyId, String plantId, String warehouseId, String itemCode, String manufactureName, String lineReferenceNo, String languageId, String putAwayNumber, Long deletionIndicator);

    @Modifying(clearAutomatically = true)
    @Query(value = "update tblputawayheader set is_deleted = 1 where REF_DOC_NO = :refDocNumber AND PRE_IB_NO = :preInboundNo and barcode_id = :barcodeId ",nativeQuery = true)
    void softDeleteByRefDocNo(@Param("refDocNumber") String refDocNumber,
                              @Param("preInboundNo") String preInboundNo,
                              @Param("barcodeId") String barcodeId);

    @Modifying
    @Query(value = "update tblputawayheader set is_deleted = 1 where REF_DOC_NO = :refDocNumber AND PRE_IB_NO = :preInboundNo ", nativeQuery = true)
    ...
    @Query(value = "update tblputawayheader SET status_Id = :statusId, status_text = :statusDescription " +
            "WHERE wh_id = :warehouseId AND ref_doc_no = :refDocNumber and c_id = :companyCodeId and plant_id = :plantId and " +
            "lang_id = :languageId and PACK_BARCODE = :packBarcodeId", nativeQuery = true)
    void updatePutAwayHeaderV10(@Param("warehouseId") String warehouseId,
                                @Param("companyCodeId") String companyCodeId,
                                @Param("plantId") String plantId,
                                @Param("languageId") String languageId,
                                @Param("refDocNumber") String refDocNumber,
    ...
    void deleteByCompanyCodeIdAndPlantIdAndWarehouseIdAndRefDocNumberAndPreInboundNoAndDeletionIndicator(
    ...
    @Query(value = "update tbliborder2 set putaway_header = :status, order_text = :text where " +
            "ref_document_no = :refDocNo", nativeQuery = true)
    void updatePutAwayHeader(@Param("refDocNo") String refDocNo,
    ...
    @Query(value = "select max(inv_id) inventoryId into #inv from tblinventory \n"
            ...
            + "MFR_DATE manufacturerDate, \n"
            + "EXP_DATE expiryDate, \n"
            + "PARENT_PRODUCTION_ORDER_NO parentProductionOrderNo, \n"
            + "MFR_CODE manufacturerCode, \n"
            + "MFR_NAME manufacturerName, \n"
            + "ORIGIN origin, \n"
            + "BRAND brand, \n"
            ...
            + "(COALESCE(CONVERT(VARCHAR(255), :startCreatedOn), null) IS NULL OR (ph.PA_CTD_ON between COALESCE(CONVERT(VARCHAR(255), :startCreatedOn), null) and COALESCE(CONVERT(VARCHAR(255), :endCreatedOn), null))) and\n"
            + "ph.is_deleted=0", nativeQuery = true)
    List<PutAwayHeaderImpl> findPutAwayHeaderV9(@Param("companyCodeId") List<String> companyCodeId,
                                                @Param("plantId") List<String> plantId,
                                                @Param("languageId") List<String> languageId,
                                                @Param("warehouseId") List<String> warehouseId,
                                                @Param("itemCode") List<String> itemCode,
    ...
    @Query(value = "update tblputawayheader SET status_Id = :statusId, status_text = :statusDescription " +
            "WHERE wh_id = :warehouseId AND ref_doc_no = :refDocNumber and c_id = :companyCodeId and plant_id = :plantId and " +
            "lang_id = :languageId and PACK_BARCODE = :packBarcodeId", nativeQuery = true)
    void updatePutAwayHeaderV9(@Param("warehouseId") String warehouseId,
                               @Param("companyCodeId") String companyCodeId,
                               @Param("plantId") String plantId,
                               @Param("languageId") String languageId,
                               @Param("refDocNumber") String refDocNumber,
    ...
    @Query(value = "SELECT * FROM tblputawayheader WHERE LANG_ID = :languageId AND C_ID = :companyCodeId AND PLANT_ID = :plantId AND WH_ID = :warehouseId \r\n"
            + "AND PRE_IB_NO = :preInboundNo AND REF_DOC_NO = :refDocNumber AND IS_DELETED = 0", nativeQuery = true)
    List<PutAwayHeaderV2> getPutawayHeaderV9(@Param("companyCodeId") String companyCodeId,
                                             @Param("languageId") String languageId,
                                             @Param("plantId") String plantId,
                                             @Param("warehouseId") String warehouseId,
                                             @Param("preInboundNo") String preInboundNo,
