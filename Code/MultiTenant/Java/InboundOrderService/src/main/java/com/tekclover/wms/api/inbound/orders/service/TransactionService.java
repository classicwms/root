...
//                    orderService.updateProcessStatus(inbound.getRefDocumentNo(), inbound.getInboundOrderTypeId(), status);
    ...
    public WarehouseApiResponse processInboundOrderV3(String profile) throws IllegalAccessException, InvocationTargetException, ParseException {
        WarehouseApiResponse warehouseApiResponse = new WarehouseApiResponse();
        ...
        if (!inboundList.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    log.info("InboundOrder ID : {}", inbound.getRefDocumentNo());
                    InboundHeaderV2 inboundHeader = orderService.processInboundReceivedV3(inbound.getRefDocumentNo(), inbound);

                    if (inboundHeader != null) {
                        inboundOrderV2Repository.updateIbOrderStatus(inboundHeader.getCompanyCode(), inboundHeader.getPlantId(), inboundHeader.getWarehouseId(), inboundHeader.getRefDocNumber(), 10L);
                        iterator.remove(); // Safe removal
                        warehouseApiResponse.setStatusCode("200");
                        warehouseApiResponse.setMessage("Success");
                    }
...
//                    orderService.updateProcessStatus(inbound.getRefDocumentNo(), inbound.getInboundOrderTypeId(), status);
    ...
    public WarehouseApiResponse processInboundOrderV4(String profile) throws IllegalAccessException, InvocationTargetException, ParseException {
        WarehouseApiResponse warehouseApiResponse = new WarehouseApiResponse();
        ...
        if (!inboundList.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    log.info("InboundOrder ID : {}", inbound.getRefDocumentNo());
                    InboundHeaderV2 inboundHeader = orderService.processInboundReceivedV4(inbound.getRefDocumentNo(), inbound);
