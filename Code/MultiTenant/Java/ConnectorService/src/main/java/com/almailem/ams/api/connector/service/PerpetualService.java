...
public class PerpetualService {
    ...
    private String getObTransactionServiceAuthToken() {
        return propertiesConfig.getFhOutboundOrderUrl();
    }

    private String getAlTransactionServiceAuthToken() {
...
//                    PerpetualLine updateCountedQty = perpetualLineRepository.save(updatePplCountedQty);
    ...
    public WarehouseApiResponse postPerpetualOrder(Perpetual perpetual) {
        ...
        UriComponentsBuilder builder =
                UriComponentsBuilder.fromHttpUrl(getObTransactionServiceAuthToken() + "outboundorder/stockcount/perpetual");
    ...
    public WarehouseApiResponse postPerpetualOrderAl(Perpetual perpetual) {
        AuthToken authToken = authTokenService.getALTransactionServiceAuthToken();
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        headers.add("User-Agent", "ClassicWMS RestTemplate");
        headers.add("Authorization", "Bearer " + authToken.getAccess_token());
        UriComponentsBuilder builder =
                UriComponentsBuilder.fromHttpUrl(getAlTransactionServiceAuthToken() + "warehouse/stockcount/perpetual");
        HttpEntity<?> entity = new HttpEntity<>(perpetual, headers);
        ResponseEntity<WarehouseApiResponse> result =
                getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, WarehouseApiResponse.class);
        log.info("result: " + result.getStatusCode());
