...
public class CustomerMasterService extends BaseService {
    ...
    private String getFhMasterServiceApiUrl() {
        return propertiesConfig.getFhMastersServiceUrl();
    }
...
//    }
     ...
     * @return
     */
    public WarehouseApiResponse postCustomerMasterV2(Customer customer) {
        AuthToken authToken = authTokenService.getFHMastersServiceAuthToken();
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        headers.add("User-Agent", "ClassicWMS RestTemplate");
        headers.add("Authorization", "Bearer " + authToken.getAccess_token());
        UriComponentsBuilder builder =
                UriComponentsBuilder.fromHttpUrl(getFhMasterServiceApiUrl() + "masterorder/master/customer");
        HttpEntity<?> entity = new HttpEntity<>(customer, headers);
