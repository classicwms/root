...
//            "(COALESCE(:expiryDate, null) IS NULL OR (iv.exp_date IN (:expiryDate))) and\n" +
    ...
    @Query(value = "select * from tblinventory where inv_id in (select max(inv_id) inventoryId from tblinventory " +
            "where is_deleted = 0 and bin_cl_id = 1 and c_id = :companyCodeId and " +
            ...
            "STCK_TYP_ID = 1 and inv_qty > 0 and ref_field_4 > 0 ", nativeQuery = true)
    List<InventoryV2> getInventoryAllocation(@Param("companyCodeId") String companyCodeId,
                                             @Param("plantId") String plantId,
                                             @Param("languageId") String languageId,
                                             @Param("warehouseId") String warehouseId,
                                             @Param("itemCode") String itemCode);
