...
import com.tekclover.wms.api.inbound.orders.model.warehouse.inbound.v2.InterWarehouseTransferInHeaderV2;
import com.tekclover.wms.api.inbound.orders.model.warehouse.inbound.v2.InterWarehouseTransferInLineV2;
import com.tekclover.wms.api.inbound.orders.model.warehouse.inbound.v2.InterWarehouseTransferInV2;
...
@Service
@Slf4j
public class InterWarehouseService {
...
//
    ...
    public List<InterWarehouseTransferInV2> processInboundReceivedV2(List<InterWarehouseTransferInV2> interWarehouseTransferInV2) {
        List<InterWarehouseTransferInV2> processInterWarehouseTransfer = Collections.synchronizedList(new ArrayList<>());
        log.info("Inbound Process Started for {} Inter warehouse Transfer", interWarehouseTransferInV2.size());

        ExecutorService executorService = Executors.newFixedThreadPool(8);

        try {
            for (InterWarehouseTransferInV2 warehouseTransferInV2 : interWarehouseTransferInV2) {
                InterWarehouseTransferInHeaderV2 headerV2 = warehouseTransferInV2.getInterWarehouseTransferInHeader();
                List<InterWarehouseTransferInLineV2> lineV2List = warehouseTransferInV2.getInterWarehouseTransferInLine();
                String companyCode = headerV2.getToCompanyCode();
                String plantId = headerV2.getToBranchCode();
...
//                    throw new BadRequestException("Order :" + refDocNumber + " already processed. Reprocessing can't be allowed.");
                ...
                List<CompletableFuture<Void>> futures = lineV2List.stream()
                        .map(srLineV2 -> CompletableFuture.runAsync(() -> {
                            try {
                                processSingleInterWarehouseTransferInLine(warehouseTransferInV2, srLineV2, preInboundHeader, inboundHeader, stagingHeader, grHeader,
                ...
                processInterWarehouseTransfer.add(warehouseTransferInV2);
            }
        } catch (Exception e) {
            log.error("Error processing inbound inter warehouse transfer Lines", e);
            throw new BadRequestException("Inbound Order Processing failed: " + e.getMessage());
        } finally {
            executorService.shutdown();
        }
        log.info("Inbound Process Completed for {} Inter warehouse Transfer In", processInterWarehouseTransfer.size());
        return processInterWarehouseTransfer;
    }

    private void processSingleInterWarehouseTransferInLine(InterWarehouseTransferInV2 warehouseTransferInV2, InterWarehouseTransferInLineV2 srLineV2, PreInboundHeaderEntityV2 preInboundHeader, InboundHeaderV2 inboundHeader, StagingHeaderV2 stagingHeader, GrHeaderV2 grHeader, List<ImBasicData1V2> imBasicDataList, List<PreInboundLineEntityV2> preInboundLineList, List<InboundLineV2> inboundLineList, List<StagingLineEntityV2> stagingLineList) {
        InterWarehouseTransferInHeaderV2 headerV2 = warehouseTransferInV2.getInterWarehouseTransferInHeader();
        String companyCode = headerV2.getToCompanyCode();
        String plantId = headerV2.getToBranchCode();
...
//        inboundLineV2Repository.save(inboundLine);
    ...
    private PreInboundLineEntityV2 createPreInboundLineV2(String companyCode, String plantId, String languageId, String preInboundNo, InterWarehouseTransferInHeaderV2 headerV2, InterWarehouseTransferInLineV2 srLineV2, String warehouseId, String companyDescription, String plantDescription, String warehouseDescription) {
        ...
        preInboundLine.setInboundOrderTypeId(4L);
//        preInboundLine.setCustomerCode(asnHeaderV2.getCustomerCode());
        preInboundLine.setTransferRequestType("WMS to WMS");
...
//        AuthToken authTokenForMastersService = authTokenService.getMastersServiceAuthToken();
        ...
        preInboundLine.setReferenceDocumentType("WMS to WMS");
...
//        stagingHeader.setCreatedOn(preInboundHeader.getCreatedOn());
    ...
    private PreInboundHeaderEntityV2 createPreInboundHeaderV2(String companyCode, String languageId, String plantId, String preInboundNo, InterWarehouseTransferInHeaderV2 headerV2, String warehouseId, String companyText, String plantText, String warehouseText, String mfrName) {
        ...
        preInboundHeader.setReferenceDocumentType("WMS to WMS");    // REF_DOC_TYP - Hard Coded Value "ASN"
        preInboundHeader.setInboundOrderTypeId(4L);    // IB_ORD_TYP_ID
        preInboundHeader.setRefDocDate(new Date());                // REF_DOC_DATE
        preInboundHeader.setTransferRequestType("WMS to WMS");
        ...
        preInboundHeader.setSourceBranchCode(headerV2.getToBranchCode());
        preInboundHeader.setSourceCompanyCode(headerV2.getToCompanyCode());
    ...
    private String getPreInboundNo(String warehouseId, String companyCode, String plantId, String languageId) {
        ...
        try {
            AuthToken authTokenForIDMasterService = authTokenService.getIDMasterServiceAuthToken();
            String nextRangeNumber = getNextRangeNumber(2L, companyCode, plantId, languageId, warehouseId, authTokenForIDMasterService.getAccess_token());
