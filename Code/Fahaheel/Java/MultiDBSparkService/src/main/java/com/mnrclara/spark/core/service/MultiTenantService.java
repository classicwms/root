...
public class MultiTenantService {
    ...
    Properties connBP = DatabaseConnectionUtil.getBPDatabaseConnectionProperties();

    Properties connMMF = DatabaseConnectionUtil.getMMFDatabaseConnectionProperties();
    ...
    String jdbcUrl9 = DatabaseConnectionUtil.getBPJdbcUrl();

    String jdbcUrl10 = DatabaseConnectionUtil.getMMFJdbcUrl();
    ...
    public List<BusinessPartnerV2> findBusinessPartner(FindBusinessPartner findBusinessPartner) throws Exception {
        try {
            ...
            } else if (routingDb.equals("BP")) {
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
                        .jdbc(jdbcUrl9, "(" + sqlQuery + ") as tmp", connBP);
                log.info("JDBC URL check -->{}", jdbcUrl9);
            } else if (routingDb.equals("MMF")) {
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
                        .jdbc(jdbcUrl10, "(" + sqlQuery + ") as tmp", connMMF);
...
//            }
            ...
            if (routingDb.equals("IMPEX")) {
                ...
                if (!conditions.isEmpty()) {
                    sqlQuery += " WHERE IS_DELETED = 0 AND REF_FIELD_4 > 0 AND inv_id in (select max(inv_id) from tblinventory where is_deleted = 0  group by itm_code,barcode_id,plant_id,wh_id,c_id,lang_id) AND " + String.join(" AND ", conditions);
                }
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
