...
//			log.info("result : " + result.getStatusCode());
	...
	// UPDATE
	public Boolean updateFirebaseNotificationMessage(FindNotificationMessage findNotificationMessage, String authToken) {
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
			headers.add("User-Agent", "MNRClara's RestTemplate");
	...
	public com.tekclover.wms.core.model.idmaster.Notification[] getAllNotification(String authToken) {
		try {
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
