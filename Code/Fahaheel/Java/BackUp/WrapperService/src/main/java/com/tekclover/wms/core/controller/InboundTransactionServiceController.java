...
//    Find
    ...
    public ResponseEntity<?> postWarehouseInboundAsnUploadV2(@RequestParam String companyCodeId, @RequestParam String plantId,
        ...
        if(profile != null){
            ...
            } else if (companyCodeId.equalsIgnoreCase("RPPL")) {
                response = fileStorageService.processInboundOrders(companyCodeId,plantId,languageId,warehouseId,inboundOrderTypeId,loginUserID,file);
            }
        }
...
//            throws IllegalAccessException, InvocationTargetException {
//        try {
//            WarehouseApiResponse response = transactionService.postASNV4(asnv2, authToken);
//            return new ResponseEntity<>(response, HttpStatus.OK);
//        } catch (Exception e) {
//            log.error("Exception while Inbound Order Post : " + e.toString());
//            CustomErrorResponse customErrorResponse = new CustomErrorResponse();
...
//    }
    ...
    public ResponseEntity<?> findErrorLog(@RequestBody FindErrorLog findErrorLog, @RequestParam String authToken) throws Exception {
        ErrorLog[] errorlog = transactionService.findErrorlog(findErrorLog,authToken);
        return new ResponseEntity<>(errorlog,HttpStatus.OK);
    }
    
//    //-----------------------------------barath-package-------------------------------------------
//    @ApiOperation(response = ASNV2.class, value = "Upload Purchase Order")
//    @DeleteMapping("/orders/inbound/upload/purchaseorder")
//    public ResponseEntity<?> purchaseOrderUpload(@RequestParam String companyCodeId, @RequestParam String plantId,
//                                                @RequestParam String languageId, @RequestParam String warehouseId,
//                                                @RequestParam String loginUserID,@RequestParam Long orderTypeId,
//                                                @RequestParam("file") MultipartFile file) throws Exception {
//        Map<String, String> response=null;
//        response = fileStorageService.processInboundOrders(companyCodeId,plantId,languageId,warehouseId,orderTypeId,loginUserID,file);
//        return new ResponseEntity<>(response,HttpStatus.OK);
//    }
//

    //----------------------------------------InboundOrderMobileApp----------------------------------------------------

    @ApiOperation(response =InboundOrderMobileApp.class, value = "New InboundOrder MobileApp")
    @PostMapping("inboundOrder/mobileApp")
    public ResponseEntity<?> postInboundOrderMobileApp(@RequestBody InboundOrderMobileApp inboundOrderMobileApp, @RequestParam String authToken ){
    ...
    //=====================================================Inventory-Update-Upload==========================================

    // File Upload - Update
    @ApiOperation(response = Inventory.class, value = "Update Inventory") // label for swagger
    @PostMapping("/inventory/update/upload")
    public ResponseEntity<?> uploadInventoryUpdate (@RequestParam String companyCodeId, @RequestParam String plantId,
                                                    @RequestParam String languageId, @RequestParam String warehouseId,
                                                    @RequestParam String loginUserID,@RequestParam("file") MultipartFile file) throws Exception {
        Map<String, String> response = fileStorageService.inventoryUpdateUpload(companyCodeId, plantId, languageId, warehouseId, loginUserID,file);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    //==============================================BarcodeGeneration=================================================
    @ApiOperation(response =BarcodeGeneration.class, value = "New BarcodeGeneration")
    @PostMapping("report/barcodeGeneration")
    public ResponseEntity<?> postBarcodeGeneration(@RequestBody List<AddBarcodeGeneration> barcodeGenerations, @RequestParam String authToken ){
        BarcodeGeneration[] barcode = transactionService.postBarcodeGeneration(barcodeGenerations,authToken);
    ...
    public ResponseEntity<?> processInbound(@RequestParam String preInboundNo,@RequestParam String refDocNumber,
                                            @RequestParam String authToken){
        WarehouseApiResponse response = transactionService.processInbound(preInboundNo, refDocNumber);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
//--------------------------------------------Notification V7 Knowell-----------------------------------------------------------//
    //Find
    @ApiOperation(response = StorageBinDashBoardImpl.class, value = "Get Storage Bin Dashboard count - Knowell V7") // label for swagger
    @PostMapping("/dashBoard/storageBinDashboard/v7")
    public StorageBinDashBoardImpl[] getStorageBinDashBoardV7(@RequestBody StorageBinDashBoardInput storageBinDashBoardInput,
                                                            @RequestParam String authToken) throws Exception {
        return transactionService.getStorageBinDashBoardV7(storageBinDashBoardInput, authToken);
    }
