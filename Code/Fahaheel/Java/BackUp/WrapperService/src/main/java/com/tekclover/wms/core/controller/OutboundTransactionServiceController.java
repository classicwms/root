...
//    }
    ...
    public ResponseEntity<?> doReversal(@RequestParam String refDocNumber, @RequestParam String itemCode,
            ...
            throws IllegalAccessException, InvocationTargetException {
        OutboundReversal[] deliveryLines = outboundTransactionService.doReversal(refDocNumber, itemCode, loginUserID,
                                            companyCodeId, plantId, warehouseId, authToken);
        log.info("deliveryLines : " + deliveryLines);
        return new ResponseEntity<>(deliveryLines, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> postSalesOrderUploadV3(@RequestParam(required = false) String companyCodeId, @RequestParam(required = false) String plantId,
        ...
        if(profile != null) {
            ...
            if(companyCodeId.equals("21")){
                ...
                response = fileStorageService.productionOrderUploadV6(companyCodeId,plantId,languageId,warehouseId,outboundOrderTypeId,loginUserID,file);
            }
        }
...
//----------------------------------------------------FG-----------------------------------------------------------------
    ...
    public ResponseEntity<?> processOutboundFG(@RequestParam String companyCodeId,@RequestParam String plantId,@RequestParam String languageId,
                                               @RequestParam String warehouseId,@RequestParam String preOutboundNo,@RequestParam String refDocNumber,@RequestParam String loginUserID,@RequestParam String authToken){
        OrderManagementLineV2[] response = outboundTransactionService.processOutboundFG(companyCodeId,plantId,languageId,warehouseId,
                preOutboundNo,refDocNumber,loginUserID);
        return new ResponseEntity<>(response,HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> orderCancellation(@RequestBody OutboundOrderCancelInput outboundOrderCancelInput, @RequestParam String loginUserID,
                                               @RequestParam String authToken) throws java.text.ParseException {
        PreOutboundHeaderV2 orderCancelled = outboundTransactionService.orderCancellation(outboundOrderCancelInput, loginUserID, authToken);
