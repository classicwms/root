...
import com.tekclover.wms.api.outbound.transaction.model.report.FastSlowMovingDashboard;
...
//          +  " AND ol.DLV_CNF_ON between :fromDate and :toDate "
    ...
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE OutboundLine ob SET ob.statusId = :statusId, ob.deliveryQty = :deliveryQty \r\n"
            + " WHERE ob.warehouseId = :warehouseId AND \r\n "
            + " ob.refDocNumber = :refDocNumber and ob.itemCode = :itemCode")
    void updateOutboundLineStatusV5(@Param("warehouseId") String warehouseId,
                                    @Param("refDocNumber") String refDocNumber,
                                    @Param("itemCode") String itemCode,
                                    @Param("statusId") Long statusId,
                                    @Param("deliveryQty") Double deliveryQty);
