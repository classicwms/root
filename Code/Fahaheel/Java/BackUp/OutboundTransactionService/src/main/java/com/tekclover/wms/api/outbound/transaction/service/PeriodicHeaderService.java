...
public class PeriodicHeaderService extends BaseService {
    ...
    public List<PeriodicHeaderEntity> findPeriodicHeader(SearchPeriodicHeader searchPeriodicHeader)
        ...
        log.info("periodicHeader result -----> {}", periodicHeaderResults);
    ...
    private List<PeriodicHeaderEntity> convertToEntity(List<PeriodicHeader> periodicheaderList, SearchPeriodicHeader searchPeriodicHeader) {
        ...
        for (PeriodicHeader periodicheader : periodicheaderList) {
            ...
            log.info("listPeriodicLineEntity -------> {}", listPeriodicLineEntity);
...
//		BeanUtils.copyProperties(updatePeriodicHeader, dbPeriodicHeader, CommonUtils.getNullPropertyNames(updatePeriodicHeader));
     ...
     * @return
     */
    public PeriodicHeaderV2 getPeriodicHeaderV6(String companyCode, String plantId, String languageId,
                                                String warehouseId, Long cycleCountTypeId, String cycleCountNo) {
        PeriodicHeaderV2 periodicHeader =
                periodicHeaderV2Repository.getPeriodicHeaderV6(
                        companyCode, plantId, languageId, warehouseId, cycleCountTypeId, cycleCountNo);
        if (periodicHeader != null && periodicHeader.getDeletionIndicator() == 0) {
            return periodicHeader;
        }
    ...
    public List<PeriodicHeaderEntityV2> findPeriodicHeaderEntityNamratha(SearchPeriodicHeaderV2 searchPeriodicHeaderV2)
            throws ParseException, java.text.ParseException {
        if (searchPeriodicHeaderV2.getStartCreatedOn() != null && searchPeriodicHeaderV2.getEndCreatedOn() != null) {
            Date[] dates = DateUtils.addTimeToDatesForSearch(searchPeriodicHeaderV2.getStartCreatedOn(),
                    searchPeriodicHeaderV2.getEndCreatedOn());
        ...
        PeriodicHeaderV6Specification spec = new PeriodicHeaderV6Specification(searchPeriodicHeaderV2);

        // Use findAll instead of stream due to multi-DB architecture
        List<PeriodicHeaderV2> periodicHeaderV2s = periodicHeaderV2Repository.findAll(spec);
...
//			periodicheaderEntity.setPeriodicLine(listPeriodicLineEntity);
    ...
    public PeriodicHeaderV2 updatePeriodicHeaderV2(String companyCode, String plantId, String languageId, String warehouseId,
        ...
        try {
             ...
             */
            log.info("PeriodicLines fetching started.......");
            List<PeriodicLineV2> PeriodicLines = periodicLineService.getPeriodicLineV6(companyCode, plantId, languageId, warehouseId, cycleCountNo);
            log.info("PeriodicLines fetching completed ------> {}", PeriodicLines);
            long count_78 = PeriodicLines.stream().filter(a -> a.getStatusId() == 78L).count();
            long count_74 = PeriodicLines.stream().filter(a -> a.getStatusId() == 74L).count();
            ...
            statusDescription = pickupLineRepository.getStatusDescription(dbPeriodicHeader.getStatusId(), languageId);
     ...
     * @throws InvocationTargetException
     */
    public PeriodicHeaderV2 updatePeriodicHeaderFromPeriodicLineV6(String companyCodeId, String plantId, String languageId,
                                                                   String warehouseId, Long cycleCountTypeId, String cycleCountNo, String loginUserID)
            throws IllegalAccessException, InvocationTargetException {

        PeriodicHeaderV2 dbPeriodicHeader = getPeriodicHeaderV6(companyCodeId, plantId, languageId, warehouseId, cycleCountTypeId, cycleCountNo);

        List<PeriodicLineV2> PeriodicLines = periodicLineService.getPeriodicLineV6(companyCodeId, plantId, languageId, warehouseId, cycleCountNo);
        long count_78 = PeriodicLines.stream().filter(a -> a.getStatusId() == 78L).count();
        long count_74 = PeriodicLines.stream().filter(a -> a.getStatusId() == 74L).count();
        long count_47 = PeriodicLines.stream().filter(a -> a.getStatusId() == 47L).count();
        long totalCount78 = count_78 + count_47;
     ...
     * @return
     */
    public PeriodicHeaderV2 getPeriodicHeaderV6(String companyCode, String plantId, String languageId, String warehouseId, String cycleCountNo) {
        PeriodicHeaderV2 periodicHeader = periodicHeaderV2Repository.getPeriodicHeader(
                companyCode, plantId, languageId, warehouseId, cycleCountNo);
        return periodicHeader;
    }
...
//        statusDescription = pickupLineRepository.getStatusDescription(70L, newPeriodicHeaderV2.getLanguageId());
//        newPeriodicHeaderV2.setStatusDescription(statusDescription);
//
//        IKeyValuePair description = pickupLineRepository.getDescription(newPeriodicHeaderV2.getCompanyCode(),
//                newPeriodicHeaderV2.getLanguageId(),
//                newPeriodicHeaderV2.getPlantId(),
//                newPeriodicHeaderV2.getWarehouseId());
...
//
//                    if (dbInventory.getBarcodeId() == null) {
//                        List<String> barcode = pickupLineRepository.getPartnerItemBarcode(dbInventory.getItemCode(),
...
//                statusDescription = pickupLineRepository.getStatusDescription(47L, newPeriodicHeaderV2.getLanguageId());
...
//
//                List<String> barcode = pickupLineRepository.getPartnerItemBarcode(cycleCountLine.getItemCode(),
//                        newPeriodicHeaderV2.getCompanyCode(),
//                        newPeriodicHeaderV2.getPlantId(),
//                        newPeriodicHeaderV2.getWarehouseId(),
//                        cycleCountLine.getManufacturerName(),
    ...
    public PeriodicHeaderEntityV2 processStockV4(String companyCode, String plantId, String languageId,
        ...
        try {
            ...
            for (PeriodicLineV2 dbPeriodicLine : periodicLines) {
                ...
                log.info("StorageBin fetch started.....");
                ...
                log.info("StorageBin fetch completed.....");
                if (dbStorageBin != null) {
                    periodicLineV2.setStorageSectionId(dbStorageBin.getStorageSectionId());
                    periodicLineV2.setLevelId(String.valueOf(dbStorageBin.getFloorId()));
                }
                ...
                if (dbPeriodicLine.getItemDesc() == null) {
                    log.info("imBasicData1 fetch started.....");
                    ...
                    log.info("imBasicData1 fetch completed.....");
                ...
                if (periodicLineV2.getBarcodeId() == null) {
                    log.info("barcode fetch started.....");
                ...
                periodicLineV2.setDeletionIndicator(0L);
                periodicLineV2.setCreatedBy(MFR_NAME);
                periodicLineV2.setCreatedOn(new Date());
                periodicLineV2s.add(periodicLineV2);

            }

            log.info("createdPeriodicLine fetch started.....");
            List<PeriodicLineV2> createdPeriodicLine = periodicLineV2Repository.saveAll(periodicLineV2s);
