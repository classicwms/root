...
//            "AND im.WH_ID in (:warehouseId) " +
    ...
    @Query(value = "select il.wh_id as warehouseId, il.itm_code as itemCode, 'InBound' as documentType ,il.ref_doc_no as documentNumber, il.partner_code as partnerCode, "
            ...
            + " (COALESCE(il.accept_qty,0) + COALESCE(il.damage_qty,0)) as movementQty, il.text as itemText ,il.mfr_name as manufacturerSKU, pu.cnf_st_bin as storageBin from tblinboundline il "
            + " OUTER APPLY (select TOP 1 cnf_st_bin from tblputawayline pu where pu.ref_doc_no = il.ref_doc_no and pu.itm_code = il.itm_code order by pu.cnf_st_bin) pu "
