...
//   	}
    ...
    @ApiOperation(response = StorageBinDashBoardImpl.class, value = "Get Storage Bin Dashboard count - Knowell")    // label for swagger
    @PostMapping("/storageBinDashboard/v7")
    public ResponseEntity<?> getStorageBinDashBoardV7(@RequestBody StorageBinDashBoardInput storageBinDashBoardInput) throws Exception {
        try {
            log.info("storageBinDashboard Input -----------> {}", storageBinDashBoardInput);
            DataBaseContextHolder.setCurrentDb("MT");
            String routingDb = dbConfigRepository.getDbName(storageBinDashBoardInput.getCompanyCodeId(), storageBinDashBoardInput.getPlantId(), storageBinDashBoardInput.getWarehouseId());
            ...
            DataBaseContextHolder.clear();
            DataBaseContextHolder.setCurrentDb(routingDb);
            List<StorageBinDashBoardImpl> storageBinDashBoardList = reportsService.getStorageBinDashBoardCountV7(storageBinDashBoardInput);
            return new ResponseEntity<>(storageBinDashBoardList, HttpStatus.OK);
        } finally {
            DataBaseContextHolder.clear();
        }
