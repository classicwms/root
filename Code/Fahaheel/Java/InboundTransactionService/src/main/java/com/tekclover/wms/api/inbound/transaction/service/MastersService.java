        ...
        try {
            ...
            UriComponentsBuilder builder =
                    UriComponentsBuilder.fromHttpUrl(getMastersServiceApiUrl() + "storagebin/" + warehouseId + "/bins/" + binClassId)
                            .queryParam("companyCode", warehouseId);
            ResponseEntity<StorageBin> result =
                    getRestTemplate().exchange(builder.toUriString(), HttpMethod.GET, entity, StorageBin.class);
            return result.getBody();
        } catch (Exception e) {
...
//    }
    ...
    //getStorageBinBinClassId1 - Proposed Bin
    public StorageBinV2 getStorageBinBinClassId1(StorageBinPutAway storageBinPutAway, String authToken) {
        try {
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
            headers.add("User-Agent", "Classic WMS's RestTemplate");
            ...
            HttpEntity<?> entity = new HttpEntity<>(storageBinPutAway, headers);
            UriComponentsBuilder builder =
                    UriComponentsBuilder.fromHttpUrl(getMastersServiceApiUrl() + "storagebin/putaway/binClassId/v2");
            ResponseEntity<StorageBinV2> result =
                    getRestTemplate().exchange(builder.toUriString(), HttpMethod.POST, entity, StorageBinV2.class);
            return result.getBody();
        } catch (Exception e) {
...
//    public StorageBin getStorageBinNew(String companyCode, String plantId, String languageId, String warehouseId, Long binClassID, String accessToken) {
//        try {
//            HttpHeaders headers = new HttpHeaders();
//            headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
//            headers.add("User-Agent", "Classic WMS's RestTemplate");
