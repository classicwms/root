...
public interface ReportRepository extends JpaRepository<Agreement, Long>,
	...
	@Query(value = "select distinct \n"+
			"(select string_agg(ta.agreement_number,', ') from tblstorenumber ts1 left join tblagreement ta on ta.agreement_number=ts1.agreement_number where ta.is_deleted=0 and ta.status!='Closed' and ts1.store_number=tsu.item_code) agreementNumber,\n"+
			"tsu.code_id storeNumber,ts.store_number storeId, \n"+
			"tsu.PHASE as phase, \n"+
			"tsu.ZONE as zone, \n"+
			"tsu.ROOM as room, \n"+
			"tsu.RACK as rack, \n"+
			...
			"(select tl.customer_code from tblleadcustomer tl join tblagreement ta on ta.customer_name=tl.customer_code join tblstorenumber ts2 on ta.agreement_number=ts2.agreement_number where ta.is_deleted=0 and ta.status!='Closed' and ts2.store_number=tsu.item_code) customerCode, \n"+
			"(select tl.customer_name from tblleadcustomer tl join tblagreement ta on ta.customer_name=tl.customer_code join tblstorenumber ts2 on ta.agreement_number=ts2.agreement_number where ta.is_deleted=0 and ta.status!='Closed' and ts2.store_number=tsu.item_code) customerName, \n"+
			"(select tl.mobile_number from tblleadcustomer tl join tblagreement ta on ta.customer_name=tl.customer_code join tblstorenumber ts2 on ta.agreement_number=ts2.agreement_number where ta.is_deleted=0 and ta.status!='Closed' and ts2.store_number=tsu.item_code) mobileNumber, \n"+
			"(select tl.phone_number from tblleadcustomer tl join tblagreement ta on ta.customer_name=tl.customer_code join tblstorenumber ts2 on ta.agreement_number=ts2.agreement_number where ta.is_deleted=0 and ta.status!='Closed' and ts2.store_number=tsu.item_code) phoneNumber \n"+
