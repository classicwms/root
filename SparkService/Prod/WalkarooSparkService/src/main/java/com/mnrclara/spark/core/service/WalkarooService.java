        ...
        try {
            ...
            if (!grHeaderList.isEmpty()) {
                ...
                if (!refDocSet.isEmpty()) {
                    ...
                    Dataset<Row> totalHuData = spark.read()
                            .jdbc(jdbcUrl, "(" + totalHuQuery + ") as tmp", connProp);

                    Dataset<Row> scannedHuData = spark.read()
                            .jdbc(jdbcUrl, "(" + scannedHuQuery + ") as tmp", connProp);
    ...
    public List<PreOutboundHeaderV2> findPreOutboundHeader(FindPreOutboundHeaderV2 findPreOutboundHeader) throws Exception {
        try {
            String sqlQuery = "SELECT "
                    ...
                    + "LEFT JOIN (SELECT SUM(ORD_QTY) as customerTotalQty, CUSTOMER_ID FROM tblpreoutboundline WHERE IS_DELETED = 0 AND STATUS_ID IN ('22', '39') GROUP BY CUSTOMER_ID) ct ON h.CUSTOMER_ID = ct.CUSTOMER_ID "
                    + "LEFT JOIN (SELECT SUM(ORD_QTY) as totalOrderQty, CUSTOMER_ID, REF_DOC_NO FROM tblpreoutboundline WHERE IS_DELETED = 0 AND STATUS_ID IN ('22','39') GROUP BY CUSTOMER_ID, REF_DOC_NO) cr ON h.CUSTOMER_ID = cr.CUSTOMER_ID AND h.REF_DOC_NO = cr.REF_DOC_NO ";
