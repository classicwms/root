...
import com.mnrclara.spark.core.model.threepl.GrLineV4;
import com.mnrclara.spark.core.model.threepl.InventoryV4;
import com.mnrclara.spark.core.model.threepl.PutAwayHeaderV4;
import com.mnrclara.spark.core.service.*;
import com.mnrclara.spark.core.service.joinspark.IBLineService;
import com.mnrclara.spark.core.service.joinspark.TPLGrLineJoinService;
import com.mnrclara.spark.core.service.joinspark.TPLIBHeaderJoinService;
import com.mnrclara.spark.core.service.joinspark.TPLObHeaderJoinService;
...
@SwaggerDefinition(tags = {@Tag(name = "3PL", description = "Operations related to User")})
@RequestMapping("/tpl/spark")
public class TPLSparkController {

    @Autowired
    TPLSparkPreInboundHeaderService sparkPreInboundHeaderService;

    @Autowired
    TPLSparkOrderManagementLineService sparkOrderManagementLineService;

    @Autowired
    TPLSparkContainerReceiptService sparkContainerReceiptService;

    @Autowired
    TPLSparkGrHeaderService sparkGrHeaderService;

    @Autowired
    TPLSparkPutAwayHeaderService sparkPutAwayHeaderService;

    @Autowired
    TPLSparkStagingHeaderService stagingHeaderService;

    @Autowired
    TPLSparkInboundHeaderService sparkInboundHeaderService;

    @Autowired
    TPLSparkPreOutboundHeaderService sparkPreOutboundHeaderService;

    @Autowired
    TPLSparkOutBoundReversalService sparkOutBoundReversalService;

    @Autowired
    TPLSparkOutBoundHeaderService sparkOutBoundHeaderService;

    @Autowired
    TPLSparkPickupHeaderService sparkPickupHeaderService;

    @Autowired
    TPLSparkQualityHeaderService sparkQualityHeaderService;

    @Autowired
    TPLSparkStorageBinService sparkStorageBinService;

    @Autowired
    TPLSparkPickupLineService sparkPickupLineService;

    @Autowired
    TPLSparkInventoryMovementService sparkInventoryMovementService;

    @Autowired
    TPLSparkInhouseTransferLineService sparkInhouseTransferLineService;

    @Autowired
    TPLSparkInventoryService sparkInventoryService;

    @Autowired
    TPLSparkPutAwayLineService sparkPutAwayLineService;

    @Autowired
    TPLSparkPeriodicHeaderService sparkPeriodicHeaderService;

    @Autowired
    TPLSparkPerpetualHeaderService sparkPerpetualHeaderService;

    @Autowired
    TPLSparkInboundLineService sparkInboundLineService;

    @Autowired
    TPLSparkStockReportService sparkStockReportService;

    @Autowired
    TPLSparkInhouseTransferHeaderService sparkInhouseTransferHeaderService;

    @Autowired
    TPLSparkOrderStatusReportService sparkOrderStatusReportService;

    @Autowired
    TPLSparkPerpetualLineService sparkPerpetualLineService;

    @Autowired
    TPLSparkPeriodicLineService sparkPeriodicLineService;

    @Autowired
    TPLSparkPreOutBoundLineService sparkPreOutBoundLineService;

    @Autowired
    TPLSparkStagingLineV2Service sparkStagingLineV2Service;

    @Autowired
    TPLSparkGrLineService sparkGrLineService;

    @Autowired
    TPLSparkQualityLineService sparkQualityLineService;

    @Autowired
    TPLSparkImBasicData1V3Service sparkImBasicData1V3Service;

    @Autowired
    TPLSparkImBasicData1V5Service sparkImBasicData1V5Service;

    @Autowired
    TPLSparkBusinessPartnerService sparkBusinessPartnerService;

    @Autowired
    TPLIBHeaderJoinService ibHeaderJoinService;
    ...
    @Autowired
    TPLSparkInventoryServiceV3 sparkInventoryV3Service;

    @Autowired
    TPLSparkPutAwayLineNewServiceV2 sparkPutAwayLineNewServiceV2;

    @Autowired
    TPLSparkInboundLineServiceV3 sparkInboundLineServiceV3;

    @Autowired
    TPLSparkPickUpLineV3Service sparkPickUpLineV3Service;

    @Autowired
    TPLSparkOutboundLineV3Service sparkOutboundLineV3Service;

    @Autowired
    TPLGrLineJoinService grLineJoinService;

    @Autowired
    TPLObHeaderJoinService obHeaderJoinService;
    ...
    public ResponseEntity<?> findPutawayHeaderV2(@RequestBody SearchPutAwayHeaderV2 searchPutAwayHeaderV2) throws Exception {
        List<PutAwayHeaderV4> findPutAwayHeaderV2 = sparkPutAwayHeaderService.findPutAwayHeaderV2(searchPutAwayHeaderV2);
    ...
    public ResponseEntity<?> findGrLineV2(@RequestBody FindGrLineV2 findGrLineV2) throws Exception {
        List<GrLineV4> stagingLineV2 = sparkGrLineService.findGrLineV2(findGrLineV2);
        return new ResponseEntity<>(stagingLineV2, HttpStatus.OK);
    }
...
//    }
    ...
    public ResponseEntity<?> getInventory(@RequestBody FindInventoryV3 findInventory) {
        List<InventoryV4> inventories = sparkInventoryV3Service.getInventory(findInventory);
    ...
    public ResponseEntity<?> getOutboundLine(@RequestBody FindOutboundLineV2 findOutBoundHeader) {
        List<OutboundLineV2> outboundLineV2s = sparkOutboundLineV3Service.getOutBoundLineV3(findOutBoundHeader);
