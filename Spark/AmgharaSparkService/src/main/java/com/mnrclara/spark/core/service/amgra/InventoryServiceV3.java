package com.mnrclara.spark.core.service.amgra;
...
public class InventoryServiceV3 {
    ...
    public List<InventoryV3> getInventory(FindInventoryV3 findInventory) {
        String sqlQuery = "SELECT INV_ID, LANG_ID, C_ID, PLANT_ID, WH_ID, " +
                ...
                "ORIGIN, BRAND, REF_DOC_NO, C_TEXT, PLANT_TEXT, WH_TEXT, STCK_TYP_TEXT, STATUS_TEXT FROM tblinventory WITH (NOLOCK)";
        ...
        if (!conditions.isEmpty()) {
            ...
            sqlQuery += " WHERE IS_DELETED = 0 AND REF_FIELD_4 > 0 AND inv_id in (select max(inv_id) from tblinventory WITH (NOLOCK) where is_deleted = 0 group by itm_code,mfr_name,st_bin,plant_id,wh_id,c_id,lang_id)";
        }
        ...
        Dataset<Row> data = spark.read()
                ...
                .jdbc("jdbc:sqlserver://10.10.10.61;databaseName=WMS_ALMPRD", "(" + sqlQuery + ") as tmp", connProp)
                .repartition(8);
