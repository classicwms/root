...
public class WalkarooService {
    ...
    Properties connPropCHN = DatabaseConnectionUtil.getWakCHNDatabaseConnectionProperties();
    String jdbcUrlCHN = DatabaseConnectionUtil.getWakCHNJdbcUrl();

    Properties connPropVGA = DatabaseConnectionUtil.getWakVGADatabaseConnectionProperties();
    String jdbcUrlVGA = DatabaseConnectionUtil.getWakVGAJdbcUrl();

    Properties connPropCCL = DatabaseConnectionUtil.getWakCCLDatabaseConnectionProperties();
    String jdbcUrlCCL = DatabaseConnectionUtil.getWakCCLJdbcUrl();

    Properties connPropHYD = DatabaseConnectionUtil.getWakHYDDatabaseConnectionProperties();
    String jdbcUrlHYD = DatabaseConnectionUtil.getWakHYDJdbcUrl();

    Properties connPropAHM = DatabaseConnectionUtil.getWakAHMDatabaseConnectionProperties();
    String jdbcUrlAHM = DatabaseConnectionUtil.getWakAHMJdbcUrl();
    ...
    public List<BusinessPartnerV2> findBusinessPartner(FindBusinessPartner findBusinessPartner) throws Exception {
        try {
            ...
            }  else if (routingDb.equals("VGA")) {
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
                        .jdbc(jdbcUrlVGA, "(" + sqlQuery + ") as tmp", connPropVGA);
                log.info("JDBC URL -->{}", jdbcUrlVGA);
            } else if (routingDb.equals("CCL")) {
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
                        .jdbc(jdbcUrlCCL, "(" + sqlQuery + ") as tmp", connPropCCL);
                log.info("JDBC URL -->{}", jdbcUrlCCL);
            } else if (routingDb.equals("HYD")) {
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
                        .jdbc(jdbcUrlHYD, "(" + sqlQuery + ") as tmp", connPropHYD);
                log.info("JDBC URL -->{}", jdbcUrlHYD);
            } else if (routingDb.equals("AHM")) {
                data = spark.read()
                        .option("fetchSize", "10000")
                        .option("pushDownloadPredicate", true)
                        .jdbc(jdbcUrlAHM, "(" + sqlQuery + ") as tmp", connPropAHM);
                log.info("JDBC URL -->{}", jdbcUrlAHM);
            }
...
//                    } else if (routingDb.equals("CMP")) {
                        ...
                        totalHuData = spark.read()
                                .jdbc(jdbcUrlVGA, "(" + totalHuQuery + ") as tmp", connPropVGA);

                        scannedHuData = spark.read()
                                .jdbc(jdbcUrlVGA, "(" + scannedHuQuery + ") as tmp", connPropVGA);
                    }  else if (routingDb.equals("CCL")) {
                        totalHuData = spark.read()
                                .jdbc(jdbcUrlCCL, "(" + totalHuQuery + ") as tmp", connPropCCL);

                        scannedHuData = spark.read()
                                .jdbc(jdbcUrlCCL, "(" + scannedHuQuery + ") as tmp", connPropCCL);
                    } else if (routingDb.equals("HYD")) {
                        totalHuData = spark.read()
                                .jdbc(jdbcUrlHYD, "(" + totalHuQuery + ") as tmp", connPropHYD);

                        scannedHuData = spark.read()
                                .jdbc(jdbcUrlHYD, "(" + scannedHuQuery + ") as tmp", connPropHYD);
                    } else if (routingDb.equals("AHM")) {
                        totalHuData = spark.read()
                                .jdbc(jdbcUrlAHM, "(" + totalHuQuery + ") as tmp", connPropAHM);

                        scannedHuData = spark.read()
                                .jdbc(jdbcUrlAHM, "(" + scannedHuQuery + ") as tmp", connPropAHM);
                    }
                    // 7. Convert to maps
...
//                    + "MATERIAL_DOC_DATE as materialDocDate "
            ...
            ConditionUtils.addCondition(conditions, "PALLET_ID", findStagingLine.getPalletId());
            ConditionUtils.addCondition(conditions, "PARTNER_ITEM_BARCODE", findStagingLine.getBarcodeId());
            ConditionUtils.addCondition(conditions, "VEHICLE_NO", findStagingLine.getVehicleNumber());
            ...
            ConditionUtils.addDateCondition(conditions, "ST_CTD_ON", findStagingLine.getStartCreatedOn(), findStagingLine.getEndCreatedOn());
...
//                    + "LEFT JOIN (SELECT SUM(ORD_QTY) as customerTotalQty, ship_to_party, CUSTOMER_ID FROM tblpreoutboundline WHERE IS_DELETED = 0 AND STATUS_ID IN ('48', '39') GROUP BY CUSTOMER_ID,ship_to_party) ct ON h.CUSTOMER_ID = ct.CUSTOMER_ID and h.ship_to_party = ct.ship_to_party "
            ...
            ConditionUtils.addCondition(conditions, "h.SHIP_TO_PARTY", findPreOutboundHeader.getShipToParty());
            ConditionUtils.addCondition(conditions, "h.SHIP_TO_CODE", findPreOutboundHeader.getShipToCode());
...
//            data.show();
    ...
    public List<InventoryV3> findInventoryV2(FindInventoryV2 findInventory) {
        try {
            ...
            if (!conditions.isEmpty()) {
                sqlQuery += " WHERE IS_DELETED = 0 AND REF_FIELD_4 > 0 AND inv_id in (select max(inv_id) from tblinventory where is_deleted = 0  group by itm_code,barcode_id,mfr_name,pack_barcode,plant_id,wh_id,c_id,lang_id) AND " + String.join(" AND ", conditions);
            } else {
                sqlQuery += " WHERE IS_DELETED = 0 AND REF_FIELD_4 > 0 AND inv_id in (select max(inv_id) from tblinventory where is_deleted = 0  group by itm_code,barcode_id,mfr_name,pack_barcode,plant_id,wh_id,c_id,lang_id)";
            }
...
//                "FROM tbloutboundheader";
     ...
     * @throws Exception
     */
    public List<PreOutboundHeaderNewRes> findPreOutboundHeaderNew(FindPreOutboundHeaderV2 findPreOutboundHeader) throws Exception {
        try {
            DataBaseContextHolder.setCurrentDb("WK");
            String routingDb = dbConfigRepository.getDbName(findPreOutboundHeader.getCompanyCodeId(), findPreOutboundHeader.getPlantId(), findPreOutboundHeader.getWarehouseId());
            log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
...
//                    + "h.SHIP_TO_PARTY shipToParty, "
//                    + "COALESCE(ct.customerTotalQty,0) as customerTotalQty, "
//                    + "COALESCE(cr.totalQty,0) as totalQty "
//                    + "FROM tblpreoutboundheader h "
//                    + "LEFT JOIN (SELECT SUM(ORD_QTY) as customerTotalQty,"
//                    + " CUSTOMER_ID, ship_to_code FROM tblpreoutboundline WHERE IS_DELETED = 0 "
//                    + "AND ref_doc_no in (select ref_doc_no from tblpreoutboundheader where is_deleted=0 and status_id in(48,39)) "
//                    + "GROUP BY CUSTOMER_ID, ship_to_code) ct ON h.CUSTOMER_ID = ct.CUSTOMER_ID and h.ship_to_code = ct.ship_to_code "
//                    + "LEFT JOIN (SELECT SUM(ORD_QTY) as totalQty, CUSTOMER_ID, REF_DOC_NO FROM tblpreoutboundline WHERE IS_DELETED = 0 "
//                    + "AND STATUS_ID IN ('48','39') GROUP BY CUSTOMER_ID, REF_DOC_NO) cr ON h.CUSTOMER_ID = cr.CUSTOMER_ID AND h.REF_DOC_NO = cr.REF_DOC_NO ";

            String sqlQuery = "SELECT "
                    + "Max(h.C_ID) as companyCodeId, "
                    + "Max(h.REF_DOC_NO) as refDocNumber, "
                    + "Max(h.PLANT_ID) as plantId, "
                    + "Max(h.WH_ID) as warehouseId, "
                    + "h.SHIP_TO_CODE as shipToCode, "
                    + "Max(h.SHIP_TO_PARTY) shipToParty, "
//                    + "COALESCE(sum(ct.customerTotalQty),0) as customerTotalQty, "
                    + "COALESCE(MAX(ct.customerTotalQty),0) as customerTotalQty, "
                    + "COALESCE(sum(cr.totalQty),0) as totalQty "
                    + "FROM tblpreoutboundheader h "
                    + "LEFT JOIN (SELECT SUM(ORD_QTY) as customerTotalQty,"
                    + " CUSTOMER_ID, ship_to_code FROM tblpreoutboundline WHERE IS_DELETED = 0 "
                    + "AND ref_doc_no in (select ref_doc_no from tblpreoutboundheader where is_deleted=0 and status_id in(48,39)) "
            ...
            sqlQuery += " group by h.ship_to_code ";
            Dataset<Row> data = null;
            if (routingDb.equals("MDU")) {
                data = spark.read()
                        .option("fetchSize", "10000")
            ...
            Encoder<PreOutboundHeaderNewRes> preOutboundHeaderEncoder = Encoders.bean(PreOutboundHeaderNewRes.class);
            Dataset<PreOutboundHeaderNewRes> dataset = data.as(preOutboundHeaderEncoder);
            return dataset.collectAsList();
        } catch (Exception e) {
            log.error("Find preOutboundHeader Spark Exception : " + e.toString());
            throw e;
     ...
     * @throws Exception
     */
    public List<PickupHeaderNewV2> findPickupHeaderV3(SearchPickupHeaderV2 findPickupHeader) throws Exception {
        try {
            DataBaseContextHolder.setCurrentDb("WK");
            String routingDb = dbConfigRepository.getDbName(findPickupHeader.getCompanyCodeId(), findPickupHeader.getPlantId(), findPickupHeader.getWarehouseId());
            log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
            ...
            ConditionUtils.addCondition(conditions, "LANG_ID", findPickupHeader.getLanguageId());
            ConditionUtils.addCondition(conditions, "REF_DOC_NO", findPickupHeader.getRefDocNumber());
            ConditionUtils.addCondition(conditions, "REF_FIELD_1", findPickupHeader.getReferenceField1());
            ConditionUtils.addCondition(conditions, "PARTNER_CODE", findPickupHeader.getPartnerCode());
            ConditionUtils.addCondition(conditions, "ITM_CODE", findPickupHeader.getItemCode());
            ConditionUtils.addCondition(conditions, "PU_NO", findPickupHeader.getPickupNumber());
            ConditionUtils.addCondition(conditions, "PROP_ST_BIN", findPickupHeader.getProposedStorageBin());
            ...
            //
            ConditionUtils.addCondition(conditions, "PARTNER_CODE", findPickupHeader.getPartnerCode());
            ConditionUtils.addCondition(conditions, "PARTNER_ITEM_BARCODE", findPickupHeader.getBarcodeId());
            ConditionUtils.addCondition(conditions, "MATERIAL_NO", findPickupHeader.getMaterialNo());
            ConditionUtils.addCondition(conditions, "PRICE_SEGMENT", findPickupHeader.getPriceSegment());
            ConditionUtils.addCondition(conditions, "ARTICLE_NO", findPickupHeader.getArticleNo());
            ConditionUtils.addCondition(conditions, "GENDER", findPickupHeader.getGender());
            ConditionUtils.addCondition(conditions, "COLOR", findPickupHeader.getColor());
            ConditionUtils.addCondition(conditions, "SIZE", findPickupHeader.getSize());
            ConditionUtils.addCondition(conditions, "NO_PAIRS", findPickupHeader.getNoPairs());
            ConditionUtils.addCondition(conditions, "REF_FIELD_2", findPickupHeader.getDocStatus());
            ConditionUtils.addCondition(conditions, "SHIP_TO_CODE", findPickupHeader.getShipToCode());
            ConditionUtils.addCondition(conditions, "SHIP_TO_PARTY", findPickupHeader.getShipToParty());
            ...
            Encoder<PickupHeaderNewV2> pickupHeaderEncoder = Encoders.bean(PickupHeaderNewV2.class);
            Dataset<PickupHeaderNewV2> dataset = data.as(pickupHeaderEncoder);
            return dataset.collectAsList();
        } catch (Exception e) {
            log.error("Find PickupHeader Spark Exception : " + e.toString());
            throw e;
     ...
     * @throws Exception
     */
    public List<NewPickupHeaderRes> findPickupHeaderV2New(SearchPickupHeaderV2 findPickupHeader) throws Exception {
        try {
            DataBaseContextHolder.setCurrentDb("WK");
            String routingDb = dbConfigRepository.getDbName(findPickupHeader.getCompanyCodeId(), findPickupHeader.getPlantId(), findPickupHeader.getWarehouseId());
            log.info("ROUTING DB FETCH FROM DB CONFIG TABLE --> {}", routingDb);
...
//                    + "STATUS_TEXT as statusDescription, "
            ...
            Encoder<NewPickupHeaderRes> pickupHeaderEncoder = Encoders.bean(NewPickupHeaderRes.class);
            Dataset<NewPickupHeaderRes> dataset = data.as(pickupHeaderEncoder);
            return dataset.collectAsList();
        } catch (Exception e) {
            log.error("Find PickupHeader Spark Exception : " + e.toString());
            throw e;
